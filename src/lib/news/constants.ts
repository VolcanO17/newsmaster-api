export const countryMappings: Record<string, string[]> = {
  US: ["US-NYT", "US-CNN", "US-HP", "US-FN", "US-R", "US-P", "US-WP", "US-LAT", "US-ABC", "US-CBS"],
  IN: ["IN-TH", "IN-TOI", "IN-HT", "IN-IE", "IN-NDTV", "IN-BS", "IN-DH", "IN-ZN", "IN-AJT", "IN-IT"],
  UK: ["UK-BBC", "UK-GU", "UK-TEL", "UK-TIMES", "UK-SKY", "UK-ITV", "UK-DM", "UK-IND", "UK-FT", "UK-REUTUK"],
  AU: ["AU-SMH", "AU-ABC", "AU-TA", "AU-PN", "AU-CT", "AU-BT", "AU-IA", "AU-BN", "AU-ID", "AU-TAUS"],
  CA: ["CA-CBCN", "CA-CTVN", "CA-FP", "CA-NP", "CA-OC", "CA-TP", "CA-TST", "CA-TSU", "CA-GM", "CA-MG"],
  DE: ["DE-ZO", "DE-FO", "DE-DW", "DE-SPI", "DE-BILD", "DE-SZ", "DE-FAZ", "DE-WELT", "DE-TAG", "DE-HB"],
  FR: ["FR-LM", "FR-LF", "FR-F24", "FR-BFMTV", "FR-LEX", "FR-LIB", "FR-LECH", "FR-LP", "FR-RTL", "FR-E1"],
  JP: ["JP-NHK", "JP-JT", "JP-ASAHI", "JP-MAIN", "JP-YOMI", "JP-NIK", "JP-KYD", "JP-JIJI", "JP-SAN", "JP-TOKYO"],
  CN: ["CN-XIN", "CN-CD", "CN-PD", "CN-GT", "CN-CGTN", "CN-CCTV", "CN-SD", "CN-BR", "CN-CO", "CN-SINA"],
  RU: ["RU-TASS", "RU-RIA", "RU-INT", "RU-KOM", "RU-IZV", "RU-RG", "RU-LENTA", "RU-RBC", "RU-VED", "RU-NG"],
  BR: ["BR-G1", "BR-FOLHA", "BR-ESTADAO", "BR-VALOR", "BR-OGLOBO", "BR-CORREIO", "BR-JORNAL", "BR-BBCBR", "BR-UOL", "BR-R7"],
  MX: ["MX-ELU", "MX-REFORMA", "MX-JORNAL", "MX-MILENIO", "MX-EXCELSIOR", "MX-ELFIN", "MX-PROCESO", "MX-ANIMAL", "MX-SINEMBARGO", "MX-QUAD"],
  AR: ["AR-CLARIN", "AR-LANACION", "AR-INF", "AR-PAGINA", "AR-CRONISTA", "AR-AMBITO", "AR-PERFIL", "AR-TN", "AR-ELDEST", "AR-BAE"],
  ZA: ["ZA-IOL", "ZA-NEWS24", "ZA-CITYPRESS", "ZA-THESTAR", "ZA-BUSINESS", "ZA-SUNDAYTIMES", "ZA-MAILGUARDIAN", "ZA-EWN", "ZA-SABC", "ZA-PRE"],
  EG: ["EG-AHRAM", "EG-DAILY", "EG-MADA", "EG-EGYPTINDEP", "EG-ALMASRY", "EG-SIS", "EG-YOUM7", "EG-ELWATAN", "EG-EGYPTTODAY", "EG-AKH"],
  TR: ["TR-HURRIYET", "TR-SABAH", "TR-MILLIYET", "TR-ZAMAN", "TR-CUMHURIYET", "TR-SOZCU", "TR-HABERTURK", "TR-NTV", "TR-BIANET", "TR-T24"],
  SA: ["SA-ARABNEWS", "SA-SPA", "SA-OKAZ", "SA-ALRIYADH", "SA-ALJAZEERA", "SA-ALWATAN", "SA-ALSHARQ", "SA-ALNAHAR", "SA-ALMADINA", "SA-ALYAUM"],
  IR: ["IR-IRNA", "IR-ISNA", "IR-TASNIM", "IR-FARS", "IR-MEHR", "IR-ILNA", "IR-SHANA", "IR-TEHRAN", "IR-PRESSTV", "IR-KHABAR"],
  PK: ["PK-DAWN", "PK-EXPRESS", "PK-NATION", "PK-ARY", "PK-GEO", "PK-SAMA", "PK-BOL", "PK-NAWA", "PK-JANG", "PK-PAKISTAN"],
  BD: ["BD-PROTHOM", "BD-DHAKA", "BD-BDNEWS", "BD-KALER", "BD-SOMOY", "BD-JUGANTOR", "BD-INQILAB", "BD-SANG", "BD-NOYADIGANTA", "BD-AMAR"],
  MY: ["MY-STAR", "MY-NST", "MY-BERNAMA", "MY-MAIL", "MY-FREEMALAYSIA", "MY-MKINI", "MY-SINAR", "MY-HARIAN", "MY-UTUSAN", "MY-KOSMO"],
  SG: ["SG-STRAITS", "SG-CNA", "SG-TODAY", "SG-BUSINESS", "SG-ZB", "SG-AsiaOne", "SG-Mothership", "SG-TNP", "SG-SG", "SG-8Days"],
  TH: ["TH-BANGKOK", "TH-NATION", "TH-KHAOSOD", "TH-MATICHON", "TH-THAIRATH", "TH-BANMUANG", "TH-PRAJ", "TH-INTERN", "TH-NAEWNA"],
  ME: ["ME-AJ"],
  SCI: ["SCI-NASA", "SCI-SCIENCEDAILY", "SCI-NATURE", "SCI-SCIENCENEWS", "SCI-SCIENCEMAG", "SCI-NEWSCIENTIST", "SCI-PHYSORG", "SCI-LIVESCIENCE", "SCI-SPACE", "SCI-ISRO"],
  SPORT: ["SPORT-CRICBUZZ", "SPORT-ESPNCRIC", "SPORT-INDIANEXPRESSSPORTS", "SPORT-TOISPORTS", "SPORT-HTSPORTS", "SPORT-INDIATODAYSPORTS", "SPORT-ZEENEWS", "SPORT-NDSPORTS", "SPORT-BUSINESSSTANDARDSPORTS", "SPORT-THEHINDUSPORTS"],
  TECH: ["TECH-TECHCRUNCH", "TECH-ARSTECHNICA", "TECH-THENEXTWEB", "TECH-ENGADGET", "TECH-GIZMODO", "TECH-TECHREPUBLIC", "TECH-ZDNET", "TECH-CNET", "TECH-INDIANEXPRESS TECH"],
  STOCK: ["STOCK-BLOOMBERG", "STOCK-REUTERSFINANCE", "STOCK-CNBC", "STOCK-FORBES", "STOCK-WSJ", "STOCK-FT", "STOCK-BUSINESSSTANDARD", "STOCK-MONEYCONTROL", "STOCK-ECONOMICTIMES", "STOCK-YAHOOFINANCE"],
  DEF: ["DEF-INDIANEXPRESS", "DEF-THEHINDU", "DEF-HT", "DEF-ND", "DEF-BS", "DEF-HAL", "DEF-MAZAGON", "DEF-DRDO", "DEF-INDIANARMY", "DEF-INDIANNAVY"],
  GEO: ["GEO-FP", "GEO-CFR", "GEO-BROOKINGS", "GEO-CARNEGIE", "GEO-ATLANTIC", "GEO-RAND", "GEO-CHATHAM", "GEO-IISS", "GEO-CSIS", "GEO-FA"],
  INTER: ["INTER-YN", "INTER-LH", "INTER-TC", "INTER-WIRED", "INTER-THEVERGE", "INTER-ARST", "INTER-IETECH", "INTER-NDTVS", "INTER-ESPNIN", "INTER-ESPN"]
};

export const newsSources: Source[] = [
  {
    "code": "INTER-YN",
    "name": "Yahoo News",
    "url": "https://www.yahoo.com/news/rss"
  },
  {
    "code": "INTER-LH",
    "name": "Life Hacker",
    "url": "https://lifehacker.com/rss"
  },
  {
    "code": "INTER-TC",
    "name": "TechCrunch",
    "url": "https://techcrunch.com/feed/"
  },
  {
    "code": "INTER-WIRED",
    "name": "Wired",
    "url": "https://www.wired.com/feed/rss"
  },
  {
    "code": "INTER-THEVERGE",
    "name": "The Verge",
    "url": "https://www.theverge.com/rss/index.xml"
  },
  {
    "code": "INTER-ARST",
    "name": "Ars Technica",
    "url": "https://feeds.arstechnica.com/arstechnica/index"
  },
  {
    "code": "INTER-IETECH",
    "name": "Indian Express Technology",
    "url": "https://indianexpress.com/section/technology/feed/"
  },
  {
    "code": "INTER-NDTVS",
    "name": "NDTV Sports",
    "url": "https://feeds.feedburner.com/ndtvnews-sports"
  },
  {
    "code": "INTER-ESPNIN",
    "name": "ESPN India",
    "url": "https://www.espn.in/rss/news"
  },
  {
    "code": "INTER-ESPN",
    "name": "ESPN",
    "url": "https://www.espn.com/espn/rss/news"
  },
  {
    "code": "INTER-ESPNCRIC",
    "name": "ESPN Cricket",
    "url": "https://www.espn.com/espn/rss/news"
  },
  {
    "code": "INTER-SKYSPORTS",
    "name": "Sky Sports",
    "url": "https://www.skysports.com/rss/12040"
  },
  {
    "code": "INTER-BBCSPORTS",
    "name": "BBC Sport",
    "url": "http://feeds.bbci.co.uk/sport/rss.xml"
  },
  {
    "code": "US-NYT",
    "name": "New York Times",
    "url": "https://rss.nytimes.com/services/xml/rss/nyt/HomePage.xml"
  },
  {
    "code": "US-CNN",
    "name": "CNN News",
    "url": "http://rss.cnn.com/rss/cnn_latest.rss"
  },
  {
    "code": "US-HP",
    "name": "Huffington Post",
    "url": "https://www.huffpost.com/section/front-page/feed"
  },
  {
    "code": "US-FN",
    "name": "Fox News",
    "url": "https://moxie.foxnews.com/google-publisher/latest.xml"
  },
  {
    "code": "US-R",
    "name": "Reuters",
    "url": "https://cdn.feedcontrol.net/8/1114-wioSIX3uu8MEj.xml"
  },
  {
    "code": "US-P",
    "name": "Politico",
    "url": "http://www.politico.com/rss/politicopicks.xml"
  },
  {
    "code": "US-WP",
    "name": "Washington Post",
    "url": "https://feeds.washingtonpost.com/rss/world"
  },
  {
    "code": "US-LAT",
    "name": "Los Angeles Times",
    "url": "https://www.latimes.com/local/rss2.0.xml"
  },
  {
    "code": "AU-SMH",
    "name": "Sydney Morning Herald - Latest News",
    "url": "https://www.smh.com.au/rss/feed.xml"
  },
  {
    "code": "AU-ABC",
    "name": "ABC News",
    "url": "https://www.abc.net.au/news/feed/1948/rss.xml"
  },
  {
    "code": "AU-TA",
    "name": "The Age - Latest News",
    "url": "https://www.theage.com.au/rss/feed.xml"
  },
  {
    "code": "AU-PN",
    "name": "PerthNow",
    "url": "https://www.perthnow.com.au/news/feed"
  },
  {
    "code": "AU-CT",
    "name": "The Canberra Times - Local News",
    "url": "https://www.canberratimes.com.au/rss.xml"
  },
  {
    "code": "AU-BT",
    "name": "Brisbane Times - Latest News",
    "url": "https://www.brisbanetimes.com.au/rss/feed.xml"
  },
  {
    "code": "AU-IA",
    "name": "Independent Australia",
    "url": "http://feeds.feedburner.com/IndependentAustralia"
  },
  {
    "code": "AU-BN",
    "name": "Business News - Latest Headlines",
    "url": "https://www.businessnews.com.au/rssfeed/latest.rss"
  },
  {
    "code": "AU-ID",
    "name": "InDaily",
    "url": "https://indaily.com.au/feed/"
  },
  {
    "code": "AU-C",
    "name": "Crikey",
    "url": "https://feeds.feedburner.com/com/rCTl"
  },
  {
    "code": "AU-MW",
    "name": "Michael West",
    "url": "https://www.michaelwest.com.au/feed/"
  },
  {
    "code": "CA-CBCN",
    "name": "CBC News",
    "url": "https://www.cbc.ca/cmlink/rss-topstories"
  },
  {
    "code": "CA-CTVN",
    "name": "CTV News",
    "url": "https://www.ctvnews.ca/rss/ctvnews-ca-top-stories-public-rss-1.822009"
  },
  {
    "code": "CA-FP",
    "name": "Financial Post",
    "url": "https://business.financialpost.com/feed/"
  },
  {
    "code": "CA-NP",
    "name": "National Post",
    "url": "https://nationalpost.com/feed/"
  },
  {
    "code": "CA-OC",
    "name": "Ottawa Citizen",
    "url": "https://ottawacitizen.com/feed/"
  },
  {
    "code": "CA-TP",
    "name": "The Province",
    "url": "https://theprovince.com/feed/"
  },
  {
    "code": "CA-TST",
    "name": "Toronto Star",
    "url": "https://www.thestar.com/content/thestar/feed.RSSManagerServlet.articles.topstories.rss"
  },
  {
    "code": "CA-TSU",
    "name": "Toronto Sun",
    "url": "https://torontosun.com/category/news/feed"
  },
  {
    "code": "DE-ZO",
    "name": "ZEIT ONLINE",
    "url": "https://www.zeit.de/gsitemaps/index.rss"
  },
  {
    "code": "DE-FO",
    "name": "FOCUS Online",
    "url": "https://rss.focus.de/fol/XML/rss_folnews.xml"
  },
  {
    "code": "DE-DW",
    "name": "Deutsche Welle",
    "url": "https://rss.dw.com/rdf/rss-en-all"
  },
  {
    "code": "IN-TH",
    "name": "The Hindu",
    "url": "https://www.thehindu.com/news/national/feeder/default.rss"
  },
  {
    "code": "IN-TOI",
    "name": "Times of India",
    "url": "https://timesofindia.indiatimes.com/rssfeedstopstories.cms"
  },
  {
    "code": "IN-HT",
    "name": "Hindustan Times",
    "url": "https://www.hindustantimes.com/rss/topnews/rssfeed.xml"
  },
  {
    "code": "IN-IE",
    "name": "Indian Express",
    "url": "https://indianexpress.com/feed/"
  },
  {
    "code": "IN-NDTV",
    "name": "NDTV",
    "url": "https://feeds.feedburner.com/ndtvnews-latest"
  },
  {
    "code": "IN-BS",
    "name": "Business Standard",
    "url": "https://www.business-standard.com/rss/latest.rss"
  },
  {
    "code": "ME-AJ",
    "name": "Al Jazeera",
    "url": "https://www.aljazeera.com/xml/rss/all.xml"
  },
  {
    "code": "UK-BBC",
    "name": "BBC News",
    "url": "http://feeds.bbci.co.uk/news/rss.xml"
  },
  {
    "code": "UK-GU",
    "name": "The Guardian",
    "url": "https://www.theguardian.com/world/rss"
  },
  {
    "code": "SCI-NASA",
    "name": "NASA News",
    "url": "https://www.nasa.gov/rss/dyn/breaking_news.rss"
  },
  {
    "code": "SCI-SCIENCEDAILY",
    "name": "Science Daily",
    "url": "https://www.sciencedaily.com/rss/all.xml"
  },
  {
    "code": "SCI-NATURE",
    "name": "Nature News",
    "url": "https://www.nature.com/nature.rss"
  },
  {
    "code": "SCI-SCIENCENEWS",
    "name": "Science News",
    "url": "https://www.sciencenews.org/feed"
  },
  {
    "code": "SCI-SCIENCEMAG",
    "name": "Science Magazine",
    "url": "https://www.sciencemag.org/rss/news_current.xml"
  },
  {
    "code": "SCI-NEWSCIENTIST",
    "name": "New Scientist",
    "url": "https://www.newscientist.com/feed/home/"
  },
  {
    "code": "SCI-PHYSORG",
    "name": "Phys.org",
    "url": "https://phys.org/rss-feed/"
  },
  {
    "code": "SCI-LIVESCIENCE",
    "name": "Live Science",
    "url": "https://www.livescience.com/feeds/all"
  },
  {
    "code": "SCI-SPACE",
    "name": "Space.com",
    "url": "https://www.space.com/feeds/all"
  },
  {
    "code": "SCI-ISRO",
    "name": "ISRO News",
    "url": "https://www.isro.gov.in/rss"
  },
  {
    "code": "SPORT-CRICBUZZ",
    "name": "Cricbuzz",
    "url": "https://www.cricbuzz.com/rss"
  },
  {
    "code": "SPORT-ESPNCRIC",
    "name": "ESPN Cricinfo",
    "url": "https://www.espncricinfo.com/rss/content/story/feeds/0.xml"
  },
  {
    "code": "SPORT-INDIANEXPRESSSPORTS",
    "name": "Indian Express Sports",
    "url": "https://indianexpress.com/section/sports/feed/"
  },
  {
    "code": "SPORT-TOISPORTS",
    "name": "Times of India Sports",
    "url": "https://timesofindia.indiatimes.com/rssfeeds/4719148.cms"
  },
  {
    "code": "SPORT-HTSPORTS",
    "name": "Hindustan Times Sports",
    "url": "https://www.hindustantimes.com/sports/rssfeed.xml"
  },
  {
    "code": "SPORT-INDIATODAYSPORTS",
    "name": "India Today Sports",
    "url": "https://www.indiatoday.in/sports/rssfeeds/-2128673031.cms"
  },
  {
    "code": "SPORT-ZEENEWS",
    "name": "Zee News Sports",
    "url": "https://zeenews.india.com/rss/sports-news.xml"
  },
  {
    "code": "SPORT-NDSPORTS",
    "name": "NDTV Sports",
    "url": "https://feeds.feedburner.com/ndtvnews-sports"
  },
  {
    "code": "SPORT-BUSINESSSTANDARDSPORTS",
    "name": "Business Standard Sports",
    "url": "https://www.business-standard.com/rss/sports-114.rss"
  },
  {
    "code": "SPORT-THEHINDUSPORTS",
    "name": "The Hindu Sports",
    "url": "https://www.thehindu.com/sport/rssfeeds/2640311/"
  },
  {
    "code": "SPORT-LIVEMINTSPORTS",
    "name": "Live Mint Sports",
    "url": "https://www.livemint.com/rss/sports"
  },
  {
    "code": "SPORT-ESPN",
    "name": "ESPN",
    "url": "https://www.espn.com/espn/rss/news"
  },
  {
    "code": "SPORT-BBCSPORT",
    "name": "BBC Sport",
    "url": "http://feeds.bbci.co.uk/sport/rss.xml"
  },
  {
    "code": "SPORT-PREMIERLEAGUE",
    "name": "Premier League",
    "url": "https://www.premierleague.com/rss.xml"
  },
  {
    "code": "SPORT-FIFA",
    "name": "FIFA",
    "url": "https://www.fifa.com/rss/index.xml"
  },
  {
    "code": "SPORT-TENNIS",
    "name": "ATP Tennis",
    "url": "https://www.atptour.com/en/rss/news.xml"
  },
  {
    "code": "TECH-TECHCRUNCH",
    "name": "TechCrunch",
    "url": "https://techcrunch.com/feed/"
  },
  {
    "code": "TECH-ARSTECHNICA",
    "name": "Ars Technica",
    "url": "https://feeds.arstechnica.com/arstechnica/index"
  },
  {
    "code": "TECH-THENEXTWEB",
    "name": "The Next Web",
    "url": "https://thenextweb.com/feed/"
  },
  {
    "code": "TECH-ENGADGET",
    "name": "Engadget",
    "url": "https://www.engadget.com/rss.xml"
  },
  {
    "code": "TECH-GIZMODO",
    "name": "Gizmodo",
    "url": "https://gizmodo.com/rss"
  },
  {
    "code": "TECH-TECHREPUBLIC",
    "name": "TechRepublic",
    "url": "https://www.techrepublic.com/rssfeeds/articles/"
  },
  {
    "code": "TECH-ZDNET",
    "name": "ZDNet",
    "url": "https://www.zdnet.com/news/rss.xml"
  },
  {
    "code": "TECH-CNET",
    "name": "CNET",
    "url": "https://www.cnet.com/rss/news/"
  },
  {
    "code": "TECH-INDIANEXPRESS TECH",
    "name": "Indian Express Technology",
    "url": "https://indianexpress.com/section/technology/feed/"
  },
  {
    "code": "STOCK-BLOOMBERG",
    "name": "Bloomberg Markets",
    "url": "https://feeds.bloomberg.com/markets/news.rss"
  },
  {
    "code": "STOCK-REUTERSFINANCE",
    "name": "Reuters Finance",
    "url": "https://feeds.reuters.com/reuters/businessNews"
  },
  {
    "code": "STOCK-CNBC",
    "name": "CNBC",
    "url": "https://www.cnbc.com/id/100003114/device/rss/rss.html"
  },
  {
    "code": "STOCK-FORBES",
    "name": "Forbes Finance",
    "url": "https://www.forbes.com/money/feed/"
  },
  {
    "code": "STOCK-WSJ",
    "name": "Wall Street Journal Markets",
    "url": "https://feeds.a.dj.com/rss/RSSMarketsMain.xml"
  },
  {
    "code": "STOCK-FT",
    "name": "Financial Times",
    "url": "https://www.ft.com/rss/home/uk"
  },
  {
    "code": "STOCK-BUSINESSSTANDARD",
    "name": "Business Standard Markets",
    "url": "https://www.business-standard.com/rss/markets-106.rss"
  },
  {
    "code": "STOCK-MONEYCONTROL",
    "name": "Moneycontrol",
    "url": "https://www.moneycontrol.com/rss/latestnews.xml"
  },
  {
    "code": "STOCK-ECONOMICTIMES",
    "name": "Economic Times Markets",
    "url": "https://economictimes.indiatimes.com/markets/rssfeeds/1977021501.cms"
  },
  {
    "code": "STOCK-YAHOOFINANCE",
    "name": "Yahoo Finance",
    "url": "https://finance.yahoo.com/rss/"
  },
  {
    "code": "STOCK-INVESTING",
    "name": "Investing.com",
    "url": "https://www.investing.com/rss/news.rss"
  },
  {
    "code": "STOCK-MARKETWATCH",
    "name": "MarketWatch",
    "url": "https://feeds.marketwatch.com/marketwatch/marketpulse/"
  },
  {
    "code": "STOCK-CNNMONEY",
    "name": "CNN Money",
    "url": "https://rss.cnn.com/rss/money_markets.rss"
  },
  {
    "code": "STOCK-SEEKINGALPHA",
    "name": "Seeking Alpha",
    "url": "https://seekingalpha.com/feed.xml"
  },
  {
    "code": "STOCK-NASDAQ",
    "name": "Nasdaq",
    "url": "https://www.nasdaq.com/feed/rssoutbound?category=Stocks"
  },
  {
    "code": "STOCK-NYSE",
    "name": "NYSE",
    "url": "https://www.nyse.com/rss/nyse.xml"
  },
  {
    "code": "STOCK-BSE",
    "name": "BSE India",
    "url": "https://www.bseindia.com/rss/feeds/News.xml"
  },
  {
    "code": "STOCK-NSE",
    "name": "NSE India",
    "url": "https://www.nseindia.com/rss/latest-news.xml"
  },
  {
    "code": "STOCK-ZEEBUSINESS",
    "name": "Zee Business",
    "url": "https://www.zeebiz.com/rss/market-news.xml"
  },
  {
    "code": "STOCK-LIVEMINT",
    "name": "Live Mint Markets",
    "url": "https://www.livemint.com/rss/markets"
  },
  {
    "code": "STOCK-INDIANEXPRESSFINANCE",
    "name": "Indian Express Finance",
    "url": "https://indianexpress.com/section/business/feed/"
  },
  {
    "code": "STOCK-HTFINANCE",
    "name": "Hindustan Times Finance",
    "url": "https://www.hindustantimes.com/business/rssfeed.xml"
  },
  {
    "code": "STOCK-ND",
    "name": "NDTV Profit",
    "url": "https://feeds.feedburner.com/ndtvprofit-latest"
  },
  {
    "code": "STOCK-BUSINESSLINE",
    "name": "Business Line",
    "url": "https://www.thehindubusinessline.com/markets/rssfeeds/1666513/"
  },
  {
    "code": "STOCK-FINANCIALEXPRESS",
    "name": "Financial Express",
    "url": "https://www.financialexpress.com/market/rssfeed.xml"
  },
  {
    "code": "DEF-INDIANEXPRESS",
    "name": "Indian Express Defence",
    "url": "https://indianexpress.com/section/india/defence/feed/"
  },
  {
    "code": "DEF-THEHINDU",
    "name": "The Hindu Defence",
    "url": "https://www.thehindu.com/news/national/defence/rssfeeds/1658055/"
  },
  {
    "code": "DEF-HT",
    "name": "Hindustan Times Defence",
    "url": "https://www.hindustantimes.com/india-news/defence/rssfeed.xml"
  },
  {
    "code": "DEF-ND",
    "name": "NDTV Defence",
    "url": "https://feeds.feedburner.com/ndtvnews-defence"
  },
  {
    "code": "DEF-BS",
    "name": "Business Standard Defence",
    "url": "https://www.business-standard.com/rss/defence-113.rss"
  },
  {
    "code": "DEF-HAL",
    "name": "Hindustan Aeronautics Limited",
    "url": "https://www.hal-india.co.in/rss"
  },
  {
    "code": "DEF-MAZAGON",
    "name": "Mazagon Dock Shipbuilders",
    "url": "https://www.mazagondock.in/rss"
  },
  {
    "code": "DEF-DRDO",
    "name": "Defence Research and Development Organisation",
    "url": "https://www.drdo.gov.in/rss"
  },
  {
    "code": "DEF-INDIANARMY",
    "name": "Indian Army",
    "url": "https://www.indianarmy.nic.in/rss"
  },
  {
    "code": "DEF-INDIANNAVY",
    "name": "Indian Navy",
    "url": "https://www.indiannavy.nic.in/rss"
  },
  {
    "code": "DEF-INDIANAIRFORCE",
    "name": "Indian Air Force",
    "url": "https://indianairforce.nic.in/rss"
  },
  {
    "code": "DEF-MODINDIA",
    "name": "Ministry of Defence India",
    "url": "https://www.mod.gov.in/rss"
  },
  {
    "code": "DEF-TOIDEFENCE",
    "name": "Times of India Defence",
    "url": "https://timesofindia.indiatimes.com/rssfeeds/3908999.cms"
  },
  {
    "code": "DEF-ECONOMICTIMESDEF",
    "name": "Economic Times Defence",
    "url": "https://economictimes.indiatimes.com/rssfeeds/1715249553.cms"
  },
  {
    "code": "DEF-FINANCIALEXPRESSDEF",
    "name": "Financial Express Defence",
    "url": "https://www.financialexpress.com/defence/rssfeed.xml"
  },
  {
    "code": "DEF-LIVEMINTDEF",
    "name": "Live Mint Defence",
    "url": "https://www.livemint.com/rss/defence"
  },
  {
    "code": "DEF-BUSINESSLINEDEF",
    "name": "Business Line Defence",
    "url": "https://www.thehindubusinessline.com/news/defence/rssfeeds/19384150/"
  },
  {
    "code": "DEF-DECCANHERALDDEF",
    "name": "Deccan Herald Defence",
    "url": "https://www.deccanherald.com/rss/defence.xml"
  },
  {
    "code": "DEF-NEWINDIANEXPRESSDEF",
    "name": "New Indian Express Defence",
    "url": "https://www.newindianexpress.com/rss/defence.xml"
  },
  {
    "code": "DEF-ASIANAGEDEF",
    "name": "The Asian Age Defence",
    "url": "https://www.asianage.com/rss/defence.xml"
  },
  {
    "code": "DEF-PIONEERDEF",
    "name": "The Pioneer Defence",
    "url": "https://www.dailypioneer.com/rss/defence.xml"
  },
  {
    "code": "DEF-TRIBUNEDEF",
    "name": "The Tribune Defence",
    "url": "https://www.tribuneindia.com/rss/defence.xml"
  },
  {
    "code": "DEF-HINDUSTANDEF",
    "name": "Hindustan Defence",
    "url": "https://www.livehindustan.com/rss/defence.xml"
  },
  {
    "code": "DEF-ZEENEWSDEF",
    "name": "Zee News Defence",
    "url": "https://zeenews.india.com/rss/defence-news.xml"
  },
  {
    "code": "DEF-ABPNLIVEDEF",
    "name": "ABP News Defence",
    "url": "https://news.abplive.com/rss/defence.xml"
  },
  {
    "code": "DEF-INDIATODAYDEF",
    "name": "India Today Defence",
    "url": "https://www.indiatoday.in/rss/1206579"
  },
  {
    "code": "DEF-AJTAKDEF",
    "name": "Aaj Tak Defence",
    "url": "https://aajtak.intoday.in/rss/defence.xml"
  },
  {
    "code": "DEF-REPUBLICDEF",
    "name": "Republic World Defence",
    "url": "https://www.republicworld.com/rss/defence.xml"
  },
  {
    "code": "DEF-ONEINDIADEF",
    "name": "Oneindia Defence",
    "url": "https://www.oneindia.com/rss/defence.xml"
  },
  {
    "code": "DEF-GREATANDHRADef",
    "name": "Great Andhra Defence",
    "url": "https://www.greatandhra.com/rss/defence.xml"
  },
  {
    "code": "DEF-SAKSHIDEF",
    "name": "Sakshi Defence",
    "url": "https://www.sakshi.com/rss/defence.xml"
  },
  {
    "code": "DEF-EENADUDEF",
    "name": "Eenadu Defence",
    "url": "https://www.eenadu.net/rss/defence.xml"
  },
  {
    "code": "DEF-ANDHRAPRAVADef",
    "name": "Andhra Pradesh Defence",
    "url": "https://www.andhraprabha.com/rss/defence.xml"
  },
  {
    "code": "DEF-MAHARASHTRADEF",
    "name": "Maharashtra Defence",
    "url": "https://www.lokmat.com/rss/defence.xml"
  },
  {
    "code": "DEF-BENGALDEF",
    "name": "Bengal Defence",
    "url": "https://www.anandabazar.com/rss/defence.xml"
  },
  {
    "code": "DEF-GUJARATDEF",
    "name": "Gujarat Defence",
    "url": "https://www.divyabhaskar.co.in/rss/defence.xml"
  },
  {
    "code": "DEF-RAJASTHANDEF",
    "name": "Rajasthan Defence",
    "url": "https://www.dainikbhaskar.com/rss/defence.xml"
  },
  {
    "code": "DEF-PUNJABDEF",
    "name": "Punjab Defence",
    "url": "https://www.jagran.com/rss/defence.xml"
  },
  {
    "code": "DEF-HARYANADEF",
    "name": "Haryana Defence",
    "url": "https://www.jagran.com/rss/defence.xml"
  },
  {
    "code": "DEF-UPDEF",
    "name": "Uttar Pradesh Defence",
    "url": "https://www.amarujala.com/rss/defence.xml"
  },
  {
    "code": "DEF-MPDEF",
    "name": "Madhya Pradesh Defence",
    "url": "https://www.bhaskar.com/rss/defence.xml"
  },
  {
    "code": "DEF-KARNATAKADEF",
    "name": "Karnataka Defence",
    "url": "https://www.vijaykarnataka.com/rss/defence.xml"
  },
  {
    "code": "DEF-TAMILNADUDEF",
    "name": "Tamil Nadu Defence",
    "url": "https://www.dinamalar.com/rss/defence.xml"
  },
  {
    "code": "DEF-KERALADef",
    "name": "Kerala Defence",
    "url": "https://www.manoramaonline.com/rss/defence.xml"
  },
  {
    "code": "DEF-DELHIDEF",
    "name": "Delhi Defence",
    "url": "https://www.navbharattimes.indiatimes.com/rss/defence.xml"
  },
  {
    "code": "DEF-JAMMUDEF",
    "name": "Jammu Defence",
    "url": "https://www.greaterkashmir.com/rss/defence.xml"
  },
  {
    "code": "DEF-LADAKHDEF",
    "name": "Ladakh Defence",
    "url": "https://www.greaterkashmir.com/rss/defence.xml"
  },
  {
    "code": "DEF-GOADEF",
    "name": "Goa Defence",
    "url": "https://www.heraldgoa.in/rss/defence.xml"
  },
  {
    "code": "DEF-PONDI_DEF",
    "name": "Pondicherry Defence",
    "url": "https://www.dinamalar.com/rss/defence.xml"
  },
  {
    "code": "DEF-CHANDIGARHDEF",
    "name": "Chandigarh Defence",
    "url": "https://www.tribuneindia.com/rss/defence.xml"
  },
  {
    "code": "DEF-DAMANDEF",
    "name": "Daman Defence",
    "url": "https://www.divyabhaskar.co.in/rss/defence.xml"
  },
  {
    "code": "DEF-DIUDEF",
    "name": "Diu Defence",
    "url": "https://www.divyabhaskar.co.in/rss/defence.xml"
  },
  {
    "code": "DEF-DADRADEF",
    "name": "Dadra Defence",
    "url": "https://www.divyabhaskar.co.in/rss/defence.xml"
  },
  {
    "code": "DEF-NAGARDEF",
    "name": "Nagar Haveli Defence",
    "url": "https://www.divyabhaskar.co.in/rss/defence.xml"
  },
  {
    "code": "DEF-LAKSHADWEEPDEF",
    "name": "Lakshadweep Defence",
    "url": "https://www.manoramaonline.com/rss/defence.xml"
  },
  {
    "code": "DEF-ANDAMANDEF",
    "name": "Andaman Defence",
    "url": "https://www.andamanchronicle.in/rss/defence.xml"
  },
  {
    "code": "DEF-NICOBARDEF",
    "name": "Nicobar Defence",
    "url": "https://www.andamanchronicle.in/rss/defence.xml"
  },
  {
    "code": "DEF-SIKKIMDEF",
    "name": "Sikkim Defence",
    "url": "https://www.thestatesman.com/rss/defence.xml"
  },
  {
    "code": "DEF-ARUNACHALDEF",
    "name": "Arunachal Pradesh Defence",
    "url": "https://www.sentinelassam.com/rss/defence.xml"
  },
  {
    "code": "DEF-NAGALANDDEF",
    "name": "Nagaland Defence",
    "url": "https://www.nagalandpost.com/rss/defence.xml"
  },
  {
    "code": "DEF-MANIPURDEF",
    "name": "Manipur Defence",
    "url": "https://www.thesangaiexpress.com/rss/defence.xml"
  },
  {
    "code": "DEF-MIZORAMDEF",
    "name": "Mizoram Defence",
    "url": "https://www.vanglaini.org/rss/defence.xml"
  },
  {
    "code": "DEF-TRIPURADEF",
    "name": "Tripura Defence",
    "url": "https://www.tripurainfoway.com/rss/defence.xml"
  },
  {
    "code": "DEF-MEGHALAYADEF",
    "name": "Meghalaya Defence",
    "url": "https://www.theshillongtimes.com/rss/defence.xml"
  },
  {
    "code": "DEF-ASSAMDEF",
    "name": "Assam Defence",
    "url": "https://www.sentinelassam.com/rss/defence.xml"
  },
  {
    "code": "DEF-WESTBENGALDEF",
    "name": "West Bengal Defence",
    "url": "https://www.anandabazar.com/rss/defence.xml"
  },
  {
    "code": "DEF-JHARKHANDDEF",
    "name": "Jharkhand Defence",
    "url": "https://www.prabhatkhabar.com/rss/defence.xml"
  },
  {
    "code": "DEF-ODISHADEF",
    "name": "Odisha Defence",
    "url": "https://www.sambad.in/rss/defence.xml"
  },
  {
    "code": "DEF-CHHATTISGARHDEF",
    "name": "Chhattisgarh Defence",
    "url": "https://www.haribhoomi.com/rss/defence.xml"
  },
  {
    "code": "DEF-BIHARDEF",
    "name": "Bihar Defence",
    "url": "https://www.hindustantimes.com/rss/defence.xml"
  },
  {
    "code": "DEF-UTTARAKHANDDEF",
    "name": "Uttarakhand Defence",
    "url": "https://www.amarujala.com/rss/defence.xml"
  },
  {
    "code": "DEF-HIMACHALDEF",
    "name": "Himachal Pradesh Defence",
    "url": "https://www.tribuneindia.com/rss/defence.xml"
  },
  {
    "code": "DEF-PUDUCHERRYDEF",
    "name": "Puducherry Defence",
    "url": "https://www.dinamalar.com/rss/defence.xml"
  },
  {
    "code": "DEF-CHANDIGARHDEF2",
    "name": "Chandigarh Defence 2",
    "url": "https://www.indianexpress.com/rss/defence.xml"
  },
  {
    "code": "DEF-LADAKHDEF2",
    "name": "Ladakh Defence 2",
    "url": "https://www.greaterkashmir.com/rss/defence.xml"
  },
  {
    "code": "DEF-JAMMUDEF2",
    "name": "Jammu Defence 2",
    "url": "https://www.greaterkashmir.com/rss/defence.xml"
  },
  {
    "code": "DEF-KASHMIRDEF",
    "name": "Kashmir Defence",
    "url": "https://www.greaterkashmir.com/rss/defence.xml"
  },
  {
    "code": "DEF-SRILANKADEF",
    "name": "Sri Lanka Defence",
    "url": "https://www.defence.lk/rss"
  },
  {
    "code": "DEF-NEPALDEF",
    "name": "Nepal Defence",
    "url": "https://www.mod.gov.np/rss"
  },
  {
    "code": "DEF-BHUTANDEF",
    "name": "Bhutan Defence",
    "url": "https://www.mod.gov.bt/rss"
  },
  {
    "code": "DEF-BANGLADESHDEF",
    "name": "Bangladesh Defence",
    "url": "https://www.mod.gov.bd/rss"
  },
  {
    "code": "DEF-MYANMARDEF",
    "name": "Myanmar Defence",
    "url": "https://www.mod.gov.mm/rss"
  },
  {
    "code": "DEF-THAILANDDEF",
    "name": "Thailand Defence",
    "url": "https://www.mod.go.th/rss"
  },
  {
    "code": "DEF-CAMBODIADEF",
    "name": "Cambodia Defence",
    "url": "https://www.mod.gov.kh/rss"
  },
  {
    "code": "DEF-LAOSDEF",
    "name": "Laos Defence",
    "url": "https://www.mod.gov.la/rss"
  },
  {
    "code": "DEF-VIETNAMDEF",
    "name": "Vietnam Defence",
    "url": "https://www.mod.gov.vn/rss"
  },
  {
    "code": "DEF-MALAYSIADEF",
    "name": "Malaysia Defence",
    "url": "https://www.mod.gov.my/rss"
  },
  {
    "code": "DEF-SINGAPOREDEF",
    "name": "Singapore Defence",
    "url": "https://www.mindef.gov.sg/rss"
  },
  {
    "code": "DEF-INDONESIADEF",
    "name": "Indonesia Defence",
    "url": "https://www.kemhan.go.id/rss"
  },
  {
    "code": "DEF-PHILIPPINESDEF",
    "name": "Philippines Defence",
    "url": "https://www.dnd.gov.ph/rss"
  },
  {
    "code": "DEF-JAPANDEF",
    "name": "Japan Defence",
    "url": "https://www.mod.go.jp/rss"
  },
  {
    "code": "DEF-SOUTHKOREADEF",
    "name": "South Korea Defence",
    "url": "https://www.mnd.go.kr/rss"
  },
  {
    "code": "DEF-AUSTRALIADEF",
    "name": "Australia Defence",
    "url": "https://www.defence.gov.au/rss"
  },
  {
    "code": "DEF-NEWZEALANDDEF",
    "name": "New Zealand Defence",
    "url": "https://www.defence.govt.nz/rss"
  },
  {
    "code": "DEF-CANADEF",
    "name": "Canada Defence",
    "url": "https://www.canada.ca/rss/defence"
  },
  {
    "code": "DEF-UKDEF",
    "name": "UK Defence",
    "url": "https://www.gov.uk/rss/mod"
  },
  {
    "code": "DEF-FRANCEDEF",
    "name": "France Defence",
    "url": "https://www.defense.gouv.fr/rss"
  },
  {
    "code": "DEF-GERMANYDEF",
    "name": "Germany Defence",
    "url": "https://www.bmvg.de/rss"
  },
  {
    "code": "DEF-CHINADEF",
    "name": "China Defence",
    "url": "https://eng.mod.gov.cn/rss"
  },
  {
    "code": "DEF-RUSSIADEF",
    "name": "Russia Defence",
    "url": "https://eng.mil.ru/rss"
  },
  {
    "code": "DEF-PAKISTANDEF",
    "name": "Pakistan Defence",
    "url": "https://www.mod.gov.pk/rss"
  },
  {
    "code": "DEF-IRANDEF",
    "name": "Iran Defence",
    "url": "https://www.mod.ir/rss"
  },
  {
    "code": "DEF-SAUDIDEF",
    "name": "Saudi Arabia Defence",
    "url": "https://www.mod.gov.sa/rss"
  },
  {
    "code": "DEF-UAEDEF",
    "name": "UAE Defence",
    "url": "https://www.mod.gov.ae/rss"
  },
  {
    "code": "DEF-TURKEYDEF",
    "name": "Turkey Defence",
    "url": "https://www.msb.gov.tr/rss"
  },
  {
    "code": "DEF-ISRAELDEF",
    "name": "Israel Defence",
    "url": "https://www.mod.gov.il/rss"
  },
  {
    "code": "DEF-EGYPTDEF",
    "name": "Egypt Defence",
    "url": "https://www.mod.gov.eg/rss"
  },
  {
    "code": "DEF-NIGERIADEF",
    "name": "Nigeria Defence",
    "url": "https://www.defence.gov.ng/rss"
  },
  {
    "code": "DEF-SOUTHAFRICADEF",
    "name": "South Africa Defence",
    "url": "https://www.dod.mil.za/rss"
  },
  {
    "code": "DEF-BRAZILDEF",
    "name": "Brazil Defence",
    "url": "https://www.defesa.gov.br/rss"
  },
  {
    "code": "DEF-MEXICODEF",
    "name": "Mexico Defence",
    "url": "https://www.gob.mx/rss/sedena"
  },
  {
    "code": "DEF-ARGENTINADEF",
    "name": "Argentina Defence",
    "url": "https://www.argentina.gob.ar/rss/defensa"
  },
  {
    "code": "DEF-CHILEDEF",
    "name": "Chile Defence",
    "url": "https://www.defensa.cl/rss"
  },
  {
    "code": "DEF-COLOMBIADEF",
    "name": "Colombia Defence",
    "url": "https://www.mindefensa.gov.co/rss"
  },
  {
    "code": "DEF-PERUDEF",
    "name": "Peru Defence",
    "url": "https://www.gob.pe/rss/defensa"
  },
  {
    "code": "DEF-VENEZUELADEF",
    "name": "Venezuela Defence",
    "url": "https://www.mindefensa.gob.ve/rss"
  },
  {
    "code": "DEF-ECUADORDEF",
    "name": "Ecuador Defence",
    "url": "https://www.defensa.gob.ec/rss"
  },
  {
    "code": "DEF-BOLIVIADEF",
    "name": "Bolivia Defence",
    "url": "https://www.mindef.gob.bo/rss"
  },
  {
    "code": "DEF-PARAGUAYDEF",
    "name": "Paraguay Defence",
    "url": "https://www.defensa.gov.py/rss"
  },
  {
    "code": "DEF-URUGUAYDEF",
    "name": "Uruguay Defence",
    "url": "https://www.minterior.gub.uy/rss/defensa"
  },
  {
    "code": "DEF-GUYANADEF",
    "name": "Guyana Defence",
    "url": "https://www.defence.gov.gy/rss"
  },
  {
    "code": "DEF-SURINAMEDEF",
    "name": "Suriname Defence",
    "url": "https://www.defensie.gov.sr/rss"
  },
  {
    "code": "DEF-FRENCHGUIANADEF",
    "name": "French Guiana Defence",
    "url": "https://www.defense.gouv.fr/rss"
  },
  {
    "code": "DEF-BARBADOSDEF",
    "name": "Barbados Defence",
    "url": "https://www.bdfbarbados.com/rss"
  },
  {
    "code": "DEF-TRINIDADDEF",
    "name": "Trinidad and Tobago Defence",
    "url": "https://www.defence.gov.tt/rss"
  },
  {
    "code": "DEF-JAMAICADEF",
    "name": "Jamaica Defence",
    "url": "https://www.jdfmil.org/rss"
  },
  {
    "code": "DEF-HAITIDEF",
    "name": "Haiti Defence",
    "url": "https://www.defense.gouv.ht/rss"
  },
  {
    "code": "DEF-DOMINICANDEF",
    "name": "Dominican Republic Defence",
    "url": "https://www.defensa.gob.do/rss"
  },
  {
    "code": "DEF-CUBADef",
    "name": "Cuba Defence",
    "url": "https://www.mindef.cu/rss"
  },
  {
    "code": "DEF-BAHAMASDEF",
    "name": "Bahamas Defence",
    "url": "https://www.bahamasedefenceforce.com/rss"
  },
  {
    "code": "DEF-BELIZEDEF",
    "name": "Belize Defence",
    "url": "https://www.defence.gov.bz/rss"
  },
  {
    "code": "DEF-COSTARICADEF",
    "name": "Costa Rica Defence",
    "url": "https://www.defensa.go.cr/rss"
  },
  {
    "code": "DEF-ELSALVADORDEF",
    "name": "El Salvador Defence",
    "url": "https://www.defensa.gob.sv/rss"
  },
  {
    "code": "DEF-GUATEMALADEF",
    "name": "Guatemala Defence",
    "url": "https://www.mindef.mil.gt/rss"
  },
  {
    "code": "DEF-HONDURASDEF",
    "name": "Honduras Defence",
    "url": "https://www.defensa.gob.hn/rss"
  },
  {
    "code": "DEF-NICARAGUADEF",
    "name": "Nicaragua Defence",
    "url": "https://www.ejercito.mil.ni/rss"
  },
  {
    "code": "DEF-PANAMADEF",
    "name": "Panama Defence",
    "url": "https://www.defensa.gob.pa/rss"
  },
  {
    "code": "DEF-ALBANIADEF",
    "name": "Albania Defence",
    "url": "https://www.mod.gov.al/rss"
  },
  {
    "code": "DEF-ARMENIADEF",
    "name": "Armenia Defence",
    "url": "https://www.mil.am/rss"
  },
  {
    "code": "DEF-AZERBAIJANDEF",
    "name": "Azerbaijan Defence",
    "url": "https://www.mod.gov.az/rss"
  },
  {
    "code": "DEF-BELARUSDEF",
    "name": "Belarus Defence",
    "url": "https://www.mil.by/rss"
  },
  {
    "code": "DEF-BOSNIADEF",
    "name": "Bosnia and Herzegovina Defence",
    "url": "https://www.mod.gov.ba/rss"
  },
  {
    "code": "DEF-BULGARIADEF",
    "name": "Bulgaria Defence",
    "url": "https://www.mod.bg/rss"
  },
  {
    "code": "DEF-CROATIADEF",
    "name": "Croatia Defence",
    "url": "https://www.morh.hr/rss"
  },
  {
    "code": "DEF-CZECHDEF",
    "name": "Czech Republic Defence",
    "url": "https://www.army.cz/rss"
  },
  {
    "code": "DEF-ESTONIADEF",
    "name": "Estonia Defence",
    "url": "https://www.kmin.ee/rss"
  },
  {
    "code": "DEF-FINLANDDEF",
    "name": "Finland Defence",
    "url": "https://www.defmin.fi/rss"
  },
  {
    "code": "DEF-GEORGIADEF",
    "name": "Georgia Defence",
    "url": "https://www.mod.gov.ge/rss"
  },
  {
    "code": "DEF-GREECEDEF",
    "name": "Greece Defence",
    "url": "https://www.mod.mil.gr/rss"
  },
  {
    "code": "DEF-HUNGARYDEF",
    "name": "Hungary Defence",
    "url": "https://www.honvedelem.hu/rss"
  },
  {
    "code": "DEF-ICELANDDEF",
    "name": "Iceland Defence",
    "url": "https://www.government.is/rss/defense"
  },
  {
    "code": "DEF-KAZAKHSTANDEF",
    "name": "Kazakhstan Defence",
    "url": "https://www.mod.gov.kz/rss"
  },
  {
    "code": "DEF-KYRGYZSTANDEF",
    "name": "Kyrgyzstan Defence",
    "url": "https://www.mod.gov.kg/rss"
  },
  {
    "code": "DEF-LATVIADEF",
    "name": "Latvia Defence",
    "url": "https://www.mod.gov.lv/rss"
  },
  {
    "code": "DEF-LITHUANIADEF",
    "name": "Lithuania Defence",
    "url": "https://www.kam.lt/rss"
  },
  {
    "code": "DEF-LUXEMBOURGDEF",
    "name": "Luxembourg Defence",
    "url": "https://defense.public.lu/rss"
  },
  {
    "code": "DEF-MOLDOVADEF",
    "name": "Moldova Defence",
    "url": "https://www.army.md/rss"
  },
  {
    "code": "DEF-MONTENEGRODEF",
    "name": "Montenegro Defence",
    "url": "https://www.mod.gov.me/rss"
  },
  {
    "code": "DEF-NETHERLANDSDEF",
    "name": "Netherlands Defence",
    "url": "https://www.defensie.nl/rss"
  },
  {
    "code": "DEF-NORTHMACEDONIADEF",
    "name": "North Macedonia Defence",
    "url": "https://www.morm.gov.mk/rss"
  },
  {
    "code": "DEF-NORWAYDEF",
    "name": "Norway Defence",
    "url": "https://www.regjeringen.no/rss/defense"
  },
  {
    "code": "DEF-POLANDDEF",
    "name": "Poland Defence",
    "url": "https://www.mon.gov.pl/rss"
  },
  {
    "code": "DEF-PORTUGALDEF",
    "name": "Portugal Defence",
    "url": "https://www.mdn.pt/rss"
  },
  {
    "code": "DEF-ROMANIADEF",
    "name": "Romania Defence",
    "url": "https://www.mapn.ro/rss"
  },
  {
    "code": "DEF-SERBIADEF",
    "name": "Serbia Defence",
    "url": "https://www.mod.gov.rs/rss"
  },
  {
    "code": "DEF-SLOVAKIADEF",
    "name": "Slovakia Defence",
    "url": "https://www.mod.gov.sk/rss"
  },
  {
    "code": "DEF-SLOVENIADEF",
    "name": "Slovenia Defence",
    "url": "https://www.mors.si/rss"
  },
  {
    "code": "DEF-SPAINDEF",
    "name": "Spain Defence",
    "url": "https://www.defensa.gob.es/rss"
  },
  {
    "code": "DEF-SWEDENDEF",
    "name": "Sweden Defence",
    "url": "https://www.forsvarsmakten.se/rss"
  },
  {
    "code": "DEF-SWITZERLANDDEF",
    "name": "Switzerland Defence",
    "url": "https://www.vbs.admin.ch/rss"
  },
  {
    "code": "DEF-TAJIKISTANDEF",
    "name": "Tajikistan Defence",
    "url": "https://www.mod.tj/rss"
  },
  {
    "code": "DEF-TURKMENISTANDEF",
    "name": "Turkmenistan Defence",
    "url": "https://www.mod.gov.tm/rss"
  },
  {
    "code": "DEF-UKRAINEDEF",
    "name": "Ukraine Defence",
    "url": "https://www.mil.gov.ua/rss"
  },
  {
    "code": "DEF-UZBEKISTANDEF",
    "name": "Uzbekistan Defence",
    "url": "https://www.mod.uz/rss"
  },
  {
    "code": "DEF-ALGERIADEF",
    "name": "Algeria Defence",
    "url": "https://www.mdn.dz/rss"
  },
  {
    "code": "DEF-MOROCCODEF",
    "name": "Morocco Defence",
    "url": "https://www.defense.gov.ma/rss"
  },
  {
    "code": "DEF-TUNISIADEF",
    "name": "Tunisia Defence",
    "url": "https://www.defense.tn/rss"
  },
  {
    "code": "DEF-LIBYADEF",
    "name": "Libya Defence",
    "url": "https://www.mod.gov.ly/rss"
  },
  {
    "code": "DEF-ETHIOPIADEF",
    "name": "Ethiopia Defence",
    "url": "https://www.mod.gov.et/rss"
  },
  {
    "code": "DEF-KENYADEF",
    "name": "Kenya Defence",
    "url": "https://www.mod.go.ke/rss"
  },
  {
    "code": "DEF-UGANDADEF",
    "name": "Uganda Defence",
    "url": "https://www.defence.go.ug/rss"
  },
  {
    "code": "DEF-TANZANIADEF",
    "name": "Tanzania Defence",
    "url": "https://www.mod.go.tz/rss"
  },
  {
    "code": "DEF-GHANADef",
    "name": "Ghana Defence",
    "url": "https://www.mod.gov.gh/rss"
  },
  {
    "code": "DEF-SENEGALDEF",
    "name": "Senegal Defence",
    "url": "https://www.defense.gouv.sn/rss"
  },
  {
    "code": "DEF-COTEIVOIREDEF",
    "name": "Cote d'Ivoire Defence",
    "url": "https://www.defense.gouv.ci/rss"
  },
  {
    "code": "DEF-NAMIBIADEF",
    "name": "Namibia Defence",
    "url": "https://www.mod.gov.na/rss"
  },
  {
    "code": "DEF-BOTSWANADEF",
    "name": "Botswana Defence",
    "url": "https://www.gov.bw/rss/defence"
  },
  {
    "code": "DEF-ZAMBIADEF",
    "name": "Zambia Defence",
    "url": "https://www.mod.gov.zm/rss"
  },
  {
    "code": "DEF-ZIMBABWEDEF",
    "name": "Zimbabwe Defence",
    "url": "https://www.mod.gov.zw/rss"
  },
  {
    "code": "DEF-MOZAMBIQUEDEF",
    "name": "Mozambique Defence",
    "url": "https://www.defesa.gov.mz/rss"
  },
  {
    "code": "DEF-ANGOLADEF",
    "name": "Angola Defence",
    "url": "https://www.defesa.angola.gov.ao/rss"
  },
  {
    "code": "DEF-MALAWI_DEF",
    "name": "Malawi Defence",
    "url": "https://www.malawidefenceforce.mil.mw/rss"
  },
  {
    "code": "DEF-MADAGASCARDEF",
    "name": "Madagascar Defence",
    "url": "https://www.defense.gov.mg/rss"
  },
  {
    "code": "DEF-MAURITIUSDEF",
    "name": "Mauritius Defence",
    "url": "https://www.gov.mu/rss/defence"
  },
  {
    "code": "DEF-SEYCHELLESDEF",
    "name": "Seychelles Defence",
    "url": "https://www.defence.gov.sc/rss"
  },
  {
    "code": "DEF-COMOROSDEF",
    "name": "Comoros Defence",
    "url": "https://www.defense.gov.km/rss"
  },
  {
    "code": "DEF-DJIBOUTIDEF",
    "name": "Djibouti Defence",
    "url": "https://www.defense.gov.dj/rss"
  },
  {
    "code": "DEF-SOMALIADEF",
    "name": "Somalia Defence",
    "url": "https://www.mod.gov.so/rss"
  },
  {
    "code": "DEF-SUDANDEF",
    "name": "Sudan Defence",
    "url": "https://www.mod.gov.sd/rss"
  },
  {
    "code": "DEF-CHADDEF",
    "name": "Chad Defence",
    "url": "https://www.defense.td/rss"
  },
  {
    "code": "DEF-NIGERDEF",
    "name": "Niger Defence",
    "url": "https://www.defense.gouv.ne/rss"
  },
  {
    "code": "DEF-MALIDEF",
    "name": "Mali Defence",
    "url": "https://www.defense.gov.ml/rss"
  },
  {
    "code": "DEF-BURKINAFASODEF",
    "name": "Burkina Faso Defence",
    "url": "https://www.defense.gov.bf/rss"
  },
  {
    "code": "DEF-GUINEADEF",
    "name": "Guinea Defence",
    "url": "https://www.defense.gov.gn/rss"
  },
  {
    "code": "DEF-SIERRADEF",
    "name": "Sierra Leone Defence",
    "url": "https://www.mod.gov.sl/rss"
  },
  {
    "code": "DEF-LIBERIADEF",
    "name": "Liberia Defence",
    "url": "https://www.mod.gov.lr/rss"
  },
  {
    "code": "DEF-GAMBIADEF",
    "name": "Gambia Defence",
    "url": "https://www.defence.gm/rss"
  },
  {
    "code": "DEF-GUINEABISSAUDEF",
    "name": "Guinea-Bissau Defence",
    "url": "https://www.defesa.gov.gw/rss"
  },
  {
    "code": "DEF-CABOVERDEDEF",
    "name": "Cape Verde Defence",
    "url": "https://www.defesa.gov.cv/rss"
  },
  {
    "code": "DEF-SAHARADEF",
    "name": "Sahrawi Arab Democratic Republic Defence",
    "url": "https://www.rasd.defense/rss"
  },
  {
    "code": "DEF-JDN",
    "name": "Jane's Defence News",
    "url": "https://www.janes.com/feeds/rss"
  },
  {
    "code": "DEF-DN",
    "name": "Defense News",
    "url": "https://www.defensenews.com/rss/"
  },
  {
    "code": "DEF-AF",
    "name": "Air Force Magazine",
    "url": "https://www.airforcemag.com/rss/"
  },
  {
    "code": "DEF-ARMY",
    "name": "Army Times",
    "url": "https://www.armytimes.com/rss/"
  },
  {
    "code": "DEF-NAVY",
    "name": "Navy Times",
    "url": "https://www.navytimes.com/rss/"
  },
  {
    "code": "DEF-MARINE",
    "name": "Marine Corps Times",
    "url": "https://www.marinecorpstimes.com/rss/"
  },
  {
    "code": "DEF-AIRFORCE",
    "name": "Air Force Times",
    "url": "https://www.airforcetimes.com/rss/"
  },
  {
    "code": "DEF-BREAKING",
    "name": "Breaking Defense",
    "url": "https://breakingdefense.com/feed/"
  },
  {
    "code": "DEF-NATIONAL",
    "name": "National Defense Magazine",
    "url": "https://www.nationaldefensemagazine.org/rss/"
  },
  {
    "code": "DEF-MILITARY",
    "name": "Military.com",
    "url": "https://www.military.com/rss/"
  },
  {
    "code": "DEF-STRATFOR",
    "name": "Stratfor",
    "url": "https://worldview.stratfor.com/feed"
  },
  {
    "code": "DEF-JANES",
    "name": "Jane's Intelligence Review",
    "url": "https://www.janes.com/feeds/rss/intelligence"
  },
  {
    "code": "DEF-GLOBAL",
    "name": "Global Security",
    "url": "https://www.globalsecurity.org/rss/"
  },
  {
    "code": "DEF-ASPI",
    "name": "Australian Strategic Policy Institute",
    "url": "https://www.aspi.org.au/rss"
  },
  {
    "code": "DEF-RUSI",
    "name": "Royal United Services Institute",
    "url": "https://rusi.org/rss"
  },
  {
    "code": "DEF-IISS",
    "name": "International Institute for Strategic Studies",
    "url": "https://www.iiss.org/rss"
  },
  {
    "code": "DEF-CSIS",
    "name": "Center for Strategic and International Studies",
    "url": "https://www.csis.org/rss"
  },
  {
    "code": "DEF-RAND",
    "name": "RAND Corporation",
    "url": "https://www.rand.org/rss/"
  },
  {
    "code": "DEF-BROOKINGS",
    "name": "Brookings Institution Defense",
    "url": "https://www.brookings.edu/feed/?topic=defense"
  },
  {
    "code": "DEF-CFR",
    "name": "Council on Foreign Relations Defense",
    "url": "https://www.cfr.org/rss/defense"
  },
  {
    "code": "DEF-HERITAGE",
    "name": "Heritage Foundation Defense",
    "url": "https://www.heritage.org/rss/defense"
  },
  {
    "code": "DEF-CATO",
    "name": "Cato Institute Defense",
    "url": "https://www.cato.org/rss/defense"
  },
  {
    "code": "DEF-AEI",
    "name": "American Enterprise Institute Defense",
    "url": "https://www.aei.org/rss/defense"
  },
  {
    "code": "DEF-IDSA",
    "name": "Institute for Defence Studies and Analyses",
    "url": "https://www.idsa.in/rss"
  },
  {
    "code": "DEF-ORF",
    "name": "Observer Research Foundation",
    "url": "https://www.orfonline.org/rss"
  },
  {
    "code": "DEF-CIP",
    "name": "Centre for International Policy Studies",
    "url": "https://www.cips-cepi.ca/rss"
  },
  {
    "code": "DEF-SIPRI",
    "name": "Stockholm International Peace Research Institute",
    "url": "https://www.sipri.org/rss"
  },
  {
    "code": "DEF-ACADEMY",
    "name": "US Army War College",
    "url": "https://warroom.armywarcollege.edu/rss/"
  },
  {
    "code": "DEF-NAVAL",
    "name": "US Naval Institute",
    "url": "https://www.usni.org/rss"
  },
  {
    "code": "DEF-AIR",
    "name": "Air University",
    "url": "https://www.airuniversity.af.edu/rss/"
  },
  {
    "code": "DEF-COAST",
    "name": "US Coast Guard",
    "url": "https://www.dco.uscg.mil/rss/"
  },
  {
    "code": "DEF-SPACE",
    "name": "Space Force",
    "url": "https://www.spaceforce.mil/rss/"
  },
  {
    "code": "DEF-CYBER",
    "name": "Cybersecurity and Infrastructure Security Agency",
    "url": "https://www.cisa.gov/rss"
  },
  {
    "code": "DEF-DOD",
    "name": "US Department of Defense",
    "url": "https://www.defense.gov/rss/"
  },
  {
    "code": "DEF-PENTAGON",
    "name": "Pentagon Press",
    "url": "https://www.defense.gov/rss/pentagon-press.xml"
  },
  {
    "code": "DEF-OSD",
    "name": "Office of the Secretary of Defense",
    "url": "https://www.defense.gov/rss/osd.xml"
  },
  {
    "code": "DEF-JOINT",
    "name": "Joint Chiefs of Staff",
    "url": "https://www.jcs.mil/rss/"
  },
  {
    "code": "DEF-ARMYHQ",
    "name": "US Army Headquarters",
    "url": "https://www.army.mil/rss/"
  },
  {
    "code": "DEF-NAVYHQ",
    "name": "US Navy Headquarters",
    "url": "https://www.navy.mil/rss/"
  },
  {
    "code": "DEF-AFHQ",
    "name": "US Air Force Headquarters",
    "url": "https://www.af.mil/rss/"
  },
  {
    "code": "DEF-MARINEHQ",
    "name": "US Marine Corps Headquarters",
    "url": "https://www.marines.mil/rss/"
  },
  {
    "code": "DEF-COASTHQ",
    "name": "US Coast Guard Headquarters",
    "url": "https://www.uscg.mil/rss/"
  },
  {
    "code": "DEF-SPACEHQ",
    "name": "US Space Force Headquarters",
    "url": "https://www.spaceforce.mil/rss/"
  },
  {
    "code": "DEF-NATO",
    "name": "NATO",
    "url": "https://www.nato.int/rss"
  },
  {
    "code": "DEF-EU",
    "name": "European Defence Agency",
    "url": "https://www.eda.europa.eu/rss"
  },
  {
    "code": "DEF-UN",
    "name": "United Nations Peacekeeping",
    "url": "https://peacekeeping.un.org/rss"
  },
  {
    "code": "DEF-OSCE",
    "name": "Organization for Security and Co-operation in Europe",
    "url": "https://www.osce.org/rss"
  },
  {
    "code": "DEF-ARAB",
    "name": "Arab League Defense",
    "url": "https://www.lasportal.org/rss/defense"
  },
  {
    "code": "DEF-ASEAN",
    "name": "ASEAN Defense",
    "url": "https://asean.org/rss/defense"
  },
  {
    "code": "DEF-SAARC",
    "name": "SAARC Defense",
    "url": "https://saarc-sec.org/rss/defense"
  },
  {
    "code": "DEF-BRICS",
    "name": "BRICS Defense",
    "url": "https://infobrics.org/rss/defense"
  },
  {
    "code": "DEF-SCO",
    "name": "Shanghai Cooperation Organization",
    "url": "https://eng.sectsco.org/rss"
  },
  {
    "code": "DEF-QUAD",
    "name": "Quad Security Dialogue",
    "url": "https://www.state.gov/rss/quad"
  },
  {
    "code": "DEF-AUKUS",
    "name": "AUKUS",
    "url": "https://www.dfat.gov.au/rss/aukus"
  },
  {
    "code": "DEF-FIVE",
    "name": "Five Eyes Intelligence Alliance",
    "url": "https://www.fiveeyesalliance.org/rss"
  },
  {
    "code": "DEF-G7",
    "name": "G7 Defense",
    "url": "https://www.g7.org/rss/defense"
  },
  {
    "code": "DEF-G20",
    "name": "G20 Defense",
    "url": "https://www.g20.org/rss/defense"
  },
  {
    "code": "DEF-PAKISTAN",
    "name": "Pakistan Ministry of Defence",
    "url": "https://www.mod.gov.pk/rss"
  },
  {
    "code": "DEF-PAKARMY",
    "name": "Pakistan Army",
    "url": "https://www.pakistanarmy.gov.pk/rss"
  },
  {
    "code": "DEF-PAKNAVY",
    "name": "Pakistan Navy",
    "url": "https://www.paknavy.gov.pk/rss"
  },
  {
    "code": "DEF-PAKAF",
    "name": "Pakistan Air Force",
    "url": "https://www.paf.gov.pk/rss"
  },
  {
    "code": "DEF-CHINA",
    "name": "China Ministry of National Defense",
    "url": "https://eng.mod.gov.cn/rss"
  },
  {
    "code": "DEF-CHINAARMY",
    "name": "People's Liberation Army",
    "url": "https://eng.chinamil.com.cn/rss"
  },
  {
    "code": "DEF-RUSSIA",
    "name": "Russia Ministry of Defence",
    "url": "https://eng.mil.ru/rss"
  },
  {
    "code": "DEF-RUSSIANARMY",
    "name": "Russian Armed Forces",
    "url": "https://eng.mil.ru/rss/army"
  },
  {
    "code": "DEF-UKMOD",
    "name": "UK Ministry of Defence",
    "url": "https://www.gov.uk/rss/mod"
  },
  {
    "code": "DEF-UKARMY",
    "name": "British Army",
    "url": "https://www.army.mod.uk/rss"
  },
  {
    "code": "DEF-UKROYALNAVY",
    "name": "Royal Navy",
    "url": "https://www.royalnavy.mod.uk/rss"
  },
  {
    "code": "DEF-UKRAF",
    "name": "Royal Air Force",
    "url": "https://www.raf.mod.uk/rss"
  },
  {
    "code": "DEF-FRANCE",
    "name": "France Ministry of Armed Forces",
    "url": "https://www.defense.gouv.fr/rss"
  },
  {
    "code": "DEF-GERMANY",
    "name": "Germany Ministry of Defence",
    "url": "https://www.bmvg.de/rss"
  },
  {
    "code": "DEF-JAPAN",
    "name": "Japan Ministry of Defence",
    "url": "https://www.mod.go.jp/rss"
  },
  {
    "code": "DEF-SOUTHKOREA",
    "name": "South Korea Ministry of National Defense",
    "url": "https://www.mnd.go.kr/rss"
  },
  {
    "code": "DEF-ISRAEL",
    "name": "Israel Ministry of Defence",
    "url": "https://www.mod.gov.il/rss"
  },
  {
    "code": "DEF-ISRAELIDF",
    "name": "Israel Defense Forces",
    "url": "https://www.idf.il/rss"
  },
  {
    "code": "DEF-SAUDI",
    "name": "Saudi Arabia Ministry of Defence",
    "url": "https://www.mod.gov.sa/rss"
  },
  {
    "code": "DEF-UAE",
    "name": "UAE Ministry of Defence",
    "url": "https://www.mod.gov.ae/rss"
  },
  {
    "code": "DEF-TURKEY",
    "name": "Turkey Ministry of National Defence",
    "url": "https://www.msb.gov.tr/rss"
  },
  {
    "code": "DEF-IRAN",
    "name": "Iran Ministry of Defence",
    "url": "https://www.mod.ir/rss"
  },
  {
    "code": "DEF-NORTHKOREA",
    "name": "North Korea Ministry of Defence",
    "url": "https://www.mod.gov.kp/rss"
  },
  {
    "code": "DEF-BRAZIL",
    "name": "Brazil Ministry of Defence",
    "url": "https://www.defesa.gov.br/rss"
  },
  {
    "code": "DEF-MEXICO",
    "name": "Mexico Secretariat of National Defense",
    "url": "https://www.gob.mx/rss/sedena"
  },
  {
    "code": "DEF-AUSTRALIA",
    "name": "Australia Department of Defence",
    "url": "https://www.defence.gov.au/rss"
  },
  {
    "code": "DEF-CANADA",
    "name": "Canada Department of National Defence",
    "url": "https://www.canada.ca/rss/defence"
  },
  {
    "code": "DEF-SOUTHKOREAARMY",
    "name": "Republic of Korea Army",
    "url": "https://www.army.mil.kr/rss"
  },
  {
    "code": "DEF-JAPANGSDF",
    "name": "Japan Ground Self-Defense Force",
    "url": "https://www.mod.go.jp/gsdf/rss"
  },
  {
    "code": "DEF-JAPANMSDF",
    "name": "Japan Maritime Self-Defense Force",
    "url": "https://www.mod.go.jp/msdf/rss"
  },
  {
    "code": "DEF-JAPANASDF",
    "name": "Japan Air Self-Defense Force",
    "url": "https://www.mod.go.jp/asdf/rss"
  },
  {
    "code": "DEF-TURKEYARMY",
    "name": "Turkish Armed Forces",
    "url": "https://www.tsk.tr/rss"
  },
  {
    "code": "DEF-IRANARMY",
    "name": "Islamic Republic of Iran Army",
    "url": "https://www.army.ir/rss"
  },
  {
    "code": "DEF-BRAZILARMY",
    "name": "Brazilian Army",
    "url": "https://www.eb.mil.br/rss"
  },
  {
    "code": "DEF-MEXICOARMY",
    "name": "Mexican Army",
    "url": "https://www.gob.mx/rss/ejercito"
  },
  {
    "code": "DEF-AUSTRALIAARMY",
    "name": "Australian Army",
    "url": "https://www.army.gov.au/rss"
  },
  {
    "code": "DEF-CANADAARMY",
    "name": "Canadian Army",
    "url": "https://www.army-armee.forces.gc.ca/rss"
  },
  {
    "code": "DEF-INDONESIA",
    "name": "Indonesia Ministry of Defence",
    "url": "https://www.kemhan.go.id/rss"
  },
  {
    "code": "DEF-THAILAND",
    "name": "Thailand Ministry of Defence",
    "url": "https://www.mod.go.th/rss"
  },
  {
    "code": "DEF-SINGAPORE",
    "name": "Singapore Ministry of Defence",
    "url": "https://www.mindef.gov.sg/rss"
  },
  {
    "code": "DEF-PHILIPPINES",
    "name": "Philippines Department of National Defense",
    "url": "https://www.dnd.gov.ph/rss"
  },
  {
    "code": "DEF-VIETNAM",
    "name": "Vietnam Ministry of National Defence",
    "url": "https://www.mod.gov.vn/rss"
  },
  {
    "code": "DEF-MALAYSIA",
    "name": "Malaysia Ministry of Defence",
    "url": "https://www.mod.gov.my/rss"
  },
  {
    "code": "DEF-NEWZEALAND",
    "name": "New Zealand Ministry of Defence",
    "url": "https://www.defence.govt.nz/rss"
  },
  {
    "code": "DEF-SOUTHKOREAARMYHQ",
    "name": "ROK Army Headquarters",
    "url": "https://www.army.mil.kr/rss/hq"
  },
  {
    "code": "DEF-JAPANGSDFHQ",
    "name": "JGSDF Headquarters",
    "url": "https://www.mod.go.jp/gsdf/rss/hq"
  },
  {
    "code": "DEF-JAPANMSDFHQ",
    "name": "JMSDF Headquarters",
    "url": "https://www.mod.go.jp/msdf/rss/hq"
  },
  {
    "code": "DEF-JAPANASDFHQ",
    "name": "JASDF Headquarters",
    "url": "https://www.mod.go.jp/asdf/rss/hq"
  },
  {
    "code": "DEF-TURKEYARMYHQ",
    "name": "Turkish General Staff",
    "url": "https://www.tsk.tr/rss/hq"
  },
  {
    "code": "DEF-IRANARMYHQ",
    "name": "Iran Army Headquarters",
    "url": "https://www.army.ir/rss/hq"
  },
  {
    "code": "DEF-BRAZILARMYHQ",
    "name": "Brazil Army Headquarters",
    "url": "https://www.eb.mil.br/rss/hq"
  },
  {
    "code": "DEF-MEXICOARMYHQ",
    "name": "Mexican Army Headquarters",
    "url": "https://www.gob.mx/rss/ejercito/hq"
  },
  {
    "code": "DEF-AUSTRALIAARMYHQ",
    "name": "Australian Army Headquarters",
    "url": "https://www.army.gov.au/rss/hq"
  },
  {
    "code": "DEF-CANADAARMYHQ",
    "name": "Canadian Army Headquarters",
    "url": "https://www.army-armee.forces.gc.ca/rss/hq"
  },
  {
    "code": "DEF-INDONESIAHQ",
    "name": "Indonesian Armed Forces HQ",
    "url": "https://www.tni.mil.id/rss"
  },
  {
    "code": "DEF-THAILANDHQ",
    "name": "Royal Thai Armed Forces HQ",
    "url": "https://www.rtaf.mi.th/rss"
  },
  {
    "code": "DEF-SINGAPOREHQ",
    "name": "Singapore Armed Forces HQ",
    "url": "https://www.mindef.gov.sg/rss/saf"
  },
  {
    "code": "DEF-PHILIPPINESHQ",
    "name": "Armed Forces of the Philippines HQ",
    "url": "https://www.afp.mil.ph/rss"
  },
  {
    "code": "DEF-VIETNAMHQ",
    "name": "Vietnam People's Army HQ",
    "url": "https://www.qdnd.vn/rss"
  },
  {
    "code": "DEF-MALAYSIAHQ",
    "name": "Malaysian Armed Forces HQ",
    "url": "https://www.mindef.gov.my/rss/atf"
  },
  {
    "code": "DEF-NEWZEALANDHQ",
    "name": "New Zealand Defence Force HQ",
    "url": "https://www.nzdf.mil.nz/rss"
  },
  {
    "code": "DEF-BULGARIA",
    "name": "Bulgaria Ministry of Defence",
    "url": "https://www.mod.bg/rss"
  },
  {
    "code": "DEF-CROATIA",
    "name": "Croatia Ministry of Defence",
    "url": "https://www.morh.hr/rss"
  },
  {
    "code": "DEF-CZECH",
    "name": "Czech Republic Ministry of Defence",
    "url": "https://www.army.cz/rss"
  },
  {
    "code": "DEF-ESTONIA",
    "name": "Estonia Ministry of Defence",
    "url": "https://www.kmin.ee/rss"
  },
  {
    "code": "DEF-FINLAND",
    "name": "Finland Ministry of Defence",
    "url": "https://www.defmin.fi/rss"
  },
  {
    "code": "DEF-GREECE",
    "name": "Greece Ministry of National Defence",
    "url": "https://www.mod.mil.gr/rss"
  },
  {
    "code": "DEF-HUNGARY",
    "name": "Hungary Ministry of Defence",
    "url": "https://www.honvedelem.hu/rss"
  },
  {
    "code": "DEF-ICELAND",
    "name": "Iceland Ministry of Foreign Affairs Defense",
    "url": "https://www.government.is/rss/defense"
  },
  {
    "code": "DEF-LATVIA",
    "name": "Latvia Ministry of Defence",
    "url": "https://www.mod.gov.lv/rss"
  },
  {
    "code": "DEF-LITHUANIA",
    "name": "Lithuania Ministry of National Defence",
    "url": "https://www.kam.lt/rss"
  },
  {
    "code": "DEF-LUXEMBOURG",
    "name": "Luxembourg Ministry of Defence",
    "url": "https://defense.public.lu/rss"
  },
  {
    "code": "DEF-NETHERLANDS",
    "name": "Netherlands Ministry of Defence",
    "url": "https://www.defensie.nl/rss"
  },
  {
    "code": "DEF-NORWAY",
    "name": "Norway Ministry of Defence",
    "url": "https://www.regjeringen.no/rss/defense"
  },
  {
    "code": "DEF-POLAND",
    "name": "Poland Ministry of National Defence",
    "url": "https://www.mon.gov.pl/rss"
  },
  {
    "code": "DEF-PORTUGAL",
    "name": "Portugal Ministry of National Defence",
    "url": "https://www.mdn.pt/rss"
  },
  {
    "code": "DEF-ROMANIA",
    "name": "Romania Ministry of National Defence",
    "url": "https://www.mapn.ro/rss"
  },
  {
    "code": "DEF-SLOVAKIA",
    "name": "Slovakia Ministry of Defence",
    "url": "https://www.mod.gov.sk/rss"
  },
  {
    "code": "DEF-SLOVENIA",
    "name": "Slovenia Ministry of Defence",
    "url": "https://www.mors.si/rss"
  },
  {
    "code": "DEF-SPAIN",
    "name": "Spain Ministry of Defence",
    "url": "https://www.defensa.gob.es/rss"
  },
  {
    "code": "DEF-SWEDEN",
    "name": "Sweden Ministry of Defence",
    "url": "https://www.forsvarsmakten.se/rss"
  },
  {
    "code": "DEF-SWITZERLAND",
    "name": "Switzerland Federal Department of Defence",
    "url": "https://www.vbs.admin.ch/rss"
  },
  {
    "code": "DEF-UKRAINE",
    "name": "Ukraine Ministry of Defence",
    "url": "https://www.mil.gov.ua/rss"
  },
  {
    "code": "DEF-ALBANIA",
    "name": "Albania Ministry of Defence",
    "url": "https://www.mod.gov.al/rss"
  },
  {
    "code": "DEF-ARMENIA",
    "name": "Armenia Ministry of Defence",
    "url": "https://www.mil.am/rss"
  },
  {
    "code": "DEF-AZERBAIJAN",
    "name": "Azerbaijan Ministry of Defence",
    "url": "https://www.mod.gov.az/rss"
  },
  {
    "code": "DEF-BELARUS",
    "name": "Belarus Ministry of Defence",
    "url": "https://www.mil.by/rss"
  },
  {
    "code": "DEF-BOSNIA",
    "name": "Bosnia and Herzegovina Ministry of Defence",
    "url": "https://www.mod.gov.ba/rss"
  },
  {
    "code": "DEF-GEORGIA",
    "name": "Georgia Ministry of Defence",
    "url": "https://www.mod.gov.ge/rss"
  },
  {
    "code": "DEF-KAZAKHSTAN",
    "name": "Kazakhstan Ministry of Defence",
    "url": "https://www.mod.gov.kz/rss"
  },
  {
    "code": "DEF-KYRGYZSTAN",
    "name": "Kyrgyzstan Ministry of Defence",
    "url": "https://www.mod.gov.kg/rss"
  },
  {
    "code": "DEF-MOLDOVA",
    "name": "Moldova Ministry of Defence",
    "url": "https://www.army.md/rss"
  },
  {
    "code": "DEF-MONTENEGRO",
    "name": "Montenegro Ministry of Defence",
    "url": "https://www.mod.gov.me/rss"
  },
  {
    "code": "DEF-NORTHMACEDONIA",
    "name": "North Macedonia Ministry of Defence",
    "url": "https://www.morm.gov.mk/rss"
  },
  {
    "code": "DEF-SERBIA",
    "name": "Serbia Ministry of Defence",
    "url": "https://www.mod.gov.rs/rss"
  },
  {
    "code": "DEF-TAJIKISTAN",
    "name": "Tajikistan Ministry of Defence",
    "url": "https://www.mod.tj/rss"
  },
  {
    "code": "DEF-TURKMENISTAN",
    "name": "Turkmenistan Ministry of Defence",
    "url": "https://www.mod.gov.tm/rss"
  },
  {
    "code": "DEF-UZBEKISTAN",
    "name": "Uzbekistan Ministry of Defence",
    "url": "https://www.mod.uz/rss"
  },
  {
    "code": "DEF-EGYPT",
    "name": "Egypt Ministry of Defence",
    "url": "https://www.mod.gov.eg/rss"
  },
  {
    "code": "DEF-ALGERIA",
    "name": "Algeria Ministry of National Defence",
    "url": "https://www.mdn.dz/rss"
  },
  {
    "code": "DEF-MOROCCO",
    "name": "Morocco Ministry of Defence",
    "url": "https://www.defense.gov.ma/rss"
  },
  {
    "code": "DEF-TUNISIA",
    "name": "Tunisia Ministry of Defence",
    "url": "https://www.defense.tn/rss"
  },
  {
    "code": "DEF-LIBYA",
    "name": "Libya Ministry of Defence",
    "url": "https://www.mod.gov.ly/rss"
  },
  {
    "code": "DEF-NIGERIA",
    "name": "Nigeria Ministry of Defence",
    "url": "https://www.defence.gov.ng/rss"
  },
  {
    "code": "DEF-ETHIOPIA",
    "name": "Ethiopia Ministry of Defence",
    "url": "https://www.mod.gov.et/rss"
  },
  {
    "code": "DEF-KENYA",
    "name": "Kenya Ministry of Defence",
    "url": "https://www.mod.go.ke/rss"
  },
  {
    "code": "DEF-UGANDA",
    "name": "Uganda Ministry of Defence",
    "url": "https://www.defence.go.ug/rss"
  },
  {
    "code": "DEF-TANZANIA",
    "name": "Tanzania Ministry of Defence",
    "url": "https://www.mod.go.tz/rss"
  },
  {
    "code": "DEF-GHANA",
    "name": "Ghana Ministry of Defence",
    "url": "https://www.mod.gov.gh/rss"
  },
  {
    "code": "DEF-SENEGAL",
    "name": "Senegal Ministry of Defence",
    "url": "https://www.defense.gouv.sn/rss"
  },
  {
    "code": "DEF-COTEIVOIRE",
    "name": "Cote d'Ivoire Ministry of Defence",
    "url": "https://www.defense.gouv.ci/rss"
  },
  {
    "code": "DEF-NAMIBIA",
    "name": "Namibia Ministry of Defence",
    "url": "https://www.mod.gov.na/rss"
  },
  {
    "code": "DEF-BOTSWANA",
    "name": "Botswana Ministry of Defence",
    "url": "https://www.gov.bw/rss/defence"
  },
  {
    "code": "DEF-ZAMBIA",
    "name": "Zambia Ministry of Defence",
    "url": "https://www.mod.gov.zm/rss"
  },
  {
    "code": "DEF-ZIMBABWE",
    "name": "Zimbabwe Ministry of Defence",
    "url": "https://www.mod.gov.zw/rss"
  },
  {
    "code": "DEF-MOZAMBIQUE",
    "name": "Mozambique Ministry of Defence",
    "url": "https://www.defesa.gov.mz/rss"
  },
  {
    "code": "DEF-ANGOLA",
    "name": "Angola Ministry of Defence",
    "url": "https://www.defesa.angola.gov.ao/rss"
  },
  {
    "code": "DEF-MALAWI",
    "name": "Malawi Ministry of Defence",
    "url": "https://www.malawidefenceforce.mil.mw/rss"
  },
  {
    "code": "DEF-MADAGASCAR",
    "name": "Madagascar Ministry of Defence",
    "url": "https://www.defense.gov.mg/rss"
  },
  {
    "code": "DEF-MAURITIUS",
    "name": "Mauritius Ministry of Defence",
    "url": "https://www.gov.mu/rss/defence"
  },
  {
    "code": "DEF-SEYCHELLES",
    "name": "Seychelles Ministry of Defence",
    "url": "https://www.defence.gov.sc/rss"
  },
  {
    "code": "DEF-COMOROS",
    "name": "Comoros Ministry of Defence",
    "url": "https://www.defense.gov.km/rss"
  },
  {
    "code": "DEF-DJIBOUTI",
    "name": "Djibouti Ministry of Defence",
    "url": "https://www.defense.gov.dj/rss"
  },
  {
    "code": "DEF-SOMALIA",
    "name": "Somalia Ministry of Defence",
    "url": "https://www.mod.gov.so/rss"
  },
  {
    "code": "DEF-SUDAN",
    "name": "Sudan Ministry of Defence",
    "url": "https://www.mod.gov.sd/rss"
  },
  {
    "code": "DEF-CHAD",
    "name": "Chad Ministry of Defence",
    "url": "https://www.defense.td/rss"
  },
  {
    "code": "DEF-NIGER",
    "name": "Niger Ministry of Defence",
    "url": "https://www.defense.gouv.ne/rss"
  },
  {
    "code": "DEF-MALI",
    "name": "Mali Ministry of Defence",
    "url": "https://www.defense.gov.ml/rss"
  },
  {
    "code": "DEF-BURKINA",
    "name": "Burkina Faso Ministry of Defence",
    "url": "https://www.defense.gov.bf/rss"
  },
  {
    "code": "DEF-GUINEA",
    "name": "Guinea Ministry of Defence",
    "url": "https://www.defense.gov.gn/rss"
  },
  {
    "code": "DEF-SIERRA",
    "name": "Sierra Leone Ministry of Defence",
    "url": "https://www.mod.gov.sl/rss"
  },
  {
    "code": "DEF-LIBERIA",
    "name": "Liberia Ministry of Defence",
    "url": "https://www.mod.gov.lr/rss"
  },
  {
    "code": "DEF-GAMBIA",
    "name": "Gambia Ministry of Defence",
    "url": "https://www.defence.gm/rss"
  },
  {
    "code": "DEF-GUINEABISSAU",
    "name": "Guinea-Bissau Ministry of Defence",
    "url": "https://www.defesa.gov.gw/rss"
  },
  {
    "code": "DEF-CABOVERDE",
    "name": "Cape Verde Ministry of Defence",
    "url": "https://www.defesa.gov.cv/rss"
  },
  {
    "code": "DEF-SAHARA",
    "name": "Sahrawi Arab Democratic Republic Ministry of Defence",
    "url": "https://www.rasd.defense/rss"
  },
  {
    "code": "DEF-MONGOLIA",
    "name": "Mongolia Ministry of Defence",
    "url": "https://www.mod.gov.mn/rss"
  },
  {
    "code": "DEF-NEPAL",
    "name": "Nepal Ministry of Defence",
    "url": "https://www.mod.gov.np/rss"
  },
  {
    "code": "DEF-BHUTAN",
    "name": "Bhutan Ministry of Defence",
    "url": "https://www.mod.gov.bt/rss"
  },
  {
    "code": "DEF-BANGLADESH",
    "name": "Bangladesh Ministry of Defence",
    "url": "https://www.mod.gov.bd/rss"
  },
  {
    "code": "DEF-SRILANKA",
    "name": "Sri Lanka Ministry of Defence",
    "url": "https://www.defence.lk/rss"
  },
  {
    "code": "DEF-MYANMAR",
    "name": "Myanmar Ministry of Defence",
    "url": "https://www.mod.gov.mm/rss"
  },
  {
    "code": "DEF-CAMBODIA",
    "name": "Cambodia Ministry of Defence",
    "url": "https://www.mod.gov.kh/rss"
  },
  {
    "code": "DEF-LAOS",
    "name": "Laos Ministry of Defence",
    "url": "https://www.mod.gov.la/rss"
  },
  {
    "code": "DEF-TIMOR",
    "name": "East Timor Ministry of Defence",
    "url": "https://www.defesa.tl/rss"
  },
  {
    "code": "DEF-PAPUA",
    "name": "Papua New Guinea Defence Force",
    "url": "https://www.pngdf.gov.pg/rss"
  },
  {
    "code": "GEO-FP",
    "name": "Foreign Policy",
    "url": "https://foreignpolicy.com/feed/"
  },
  {
    "code": "GEO-CFR",
    "name": "Council on Foreign Relations",
    "url": "https://www.cfr.org/rss"
  },
  {
    "code": "GEO-BROOKINGS",
    "name": "Brookings Institution Foreign Policy",
    "url": "https://www.brookings.edu/feed/?topic=foreign-policy"
  },
  {
    "code": "GEO-CARNEGIE",
    "name": "Carnegie Endowment for International Peace",
    "url": "https://carnegieendowment.org/rss"
  },
  {
    "code": "GEO-ATLANTIC",
    "name": "Atlantic Council",
    "url": "https://www.atlanticcouncil.org/rss"
  },
  {
    "code": "GEO-RAND",
    "name": "RAND Corporation Foreign Policy",
    "url": "https://www.rand.org/rss/foreign-policy"
  },
  {
    "code": "GEO-CHATHAM",
    "name": "Chatham House",
    "url": "https://www.chathamhouse.org/rss"
  },
  {
    "code": "GEO-IISS",
    "name": "International Institute for Strategic Studies",
    "url": "https://www.iiss.org/rss"
  },
  {
    "code": "GEO-CSIS",
    "name": "Center for Strategic and International Studies",
    "url": "https://www.csis.org/rss"
  },
  {
    "code": "GEO-FA",
    "name": "Foreign Affairs",
    "url": "https://www.foreignaffairs.com/rss"
  },
  {
    "code": "GEO-DIPLOMAT",
    "name": "The Diplomat",
    "url": "https://thediplomat.com/feed/"
  },
  {
    "code": "GEO-EURASIA",
    "name": "Eurasia Group",
    "url": "https://www.eurasiagroup.net/rss"
  },
  {
    "code": "GEO-STRATFOR",
    "name": "Stratfor",
    "url": "https://worldview.stratfor.com/feed"
  },
  {
    "code": "GEO-GPF",
    "name": "Global Policy Forum",
    "url": "https://www.globalpolicy.org/rss"
  },
  {
    "code": "GEO-WPR",
    "name": "World Politics Review",
    "url": "https://www.worldpoliticsreview.com/feed/"
  },
  {
    "code": "GEO-RCW",
    "name": "RealClearWorld",
    "url": "https://www.realclearworld.com/rss.xml"
  },
  {
    "code": "GEO-AJ",
    "name": "Al Jazeera International",
    "url": "https://www.aljazeera.com/xml/rss/all.xml"
  },
  {
    "code": "GEO-BBCWORLD",
    "name": "BBC World Service",
    "url": "https://feeds.bbci.co.uk/news/world/rss.xml"
  },
  {
    "code": "GEO-DW",
    "name": "Deutsche Welle International",
    "url": "https://rss.dw.com/rdf/rss-en-all"
  },
  {
    "code": "GEO-FRANCE24",
    "name": "France 24",
    "url": "https://www.france24.com/en/rss"
  },
  {
    "code": "GEO-RT",
    "name": "RT News",
    "url": "https://www.rt.com/rss/"
  },
  {
    "code": "GEO-CGTN",
    "name": "CGTN",
    "url": "https://news.cgtn.com/rss/"
  },
  {
    "code": "GEO-PRESSTV",
    "name": "Press TV",
    "url": "https://www.presstv.ir/rss"
  },
  {
    "code": "GEO-TASS",
    "name": "TASS",
    "url": "https://tass.com/rss"
  },
  {
    "code": "GEO-XINHUA",
    "name": "Xinhua News Agency",
    "url": "https://www.xinhuanet.com/english/rss/worldrss.xml"
  },
  {
    "code": "GEO-ANADOLU",
    "name": "Anadolu Agency",
    "url": "https://www.aa.com.tr/en/rss/default?cat=world"
  },
  {
    "code": "GEO-MEE",
    "name": "Middle East Eye",
    "url": "https://www.middleeasteye.net/rss"
  },
  {
    "code": "GEO-SCMP",
    "name": "South China Morning Post",
    "url": "https://www.scmp.com/rss/91/feed"
  },
  {
    "code": "GEO-NIKKEI",
    "name": "Nikkei Asia",
    "url": "https://asia.nikkei.com/rss"
  },
  {
    "code": "GEO-HTINT",
    "name": "Hindustan Times International",
    "url": "https://www.hindustantimes.com/feeds/rss/world-news/rssfeed.xml"
  },
  {
    "code": "GEO-HINDUINT",
    "name": "The Hindu International",
    "url": "https://www.thehindu.com/news/international/feeder/default.rss"
  },
  {
    "code": "GEO-IEINT",
    "name": "Indian Express International",
    "url": "https://indianexpress.com/section/world/feed/"
  },
  {
    "code": "GEO-ORF",
    "name": "Observer Research Foundation",
    "url": "https://www.orfonline.org/rss"
  },
  {
    "code": "GEO-IDSA",
    "name": "Institute for Defence Studies and Analyses",
    "url": "https://www.idsa.in/rss"
  },
  {
    "code": "GEO-CPR",
    "name": "Centre for Policy Research",
    "url": "https://www.cprindia.org/rss"
  },
  {
    "code": "GEO-BROOKINGSINDIA",
    "name": "Brookings India",
    "url": "https://www.brookings.edu/research/india/rss"
  },
  {
    "code": "GEO-CARNEGIEINDIA",
    "name": "Carnegie India",
    "url": "https://carnegieindia.org/rss"
  },
  {
    "code": "GEO-GATEWAY",
    "name": "Gateway House",
    "url": "https://www.gatewayhouse.in/rss"
  },
  {
    "code": "GEO-VIF",
    "name": "Vivekananda International Foundation",
    "url": "https://www.vifindia.org/rss"
  },
  {
    "code": "GEO-ICWA",
    "name": "Indian Council of World Affairs",
    "url": "https://www.icwa.in/rss"
  },
     
         //US Additional
  {
    code: "US-ABC",
    name: "ABC News",
    url: "https://abcnews.go.com/abcnews/topstories",
  },
  {
    code: "US-CBS",
    name: "CBS News",
    url: "https://www.cbsnews.com/latest/rss/main",
  },
  {
    code: "US-NBC",
    name: "NBC News",
    url: "https://www.nbcnews.com/feed",
  },
  {
    code: "US-NPR",
    name: "NPR",
    url: "https://feeds.npr.org/1001/rss.xml",
  },
  {
    code: "US-WSJ",
    name: "The Wall Street Journal",
    url: "https://feeds.a.dj.com/rss/RSSWorldNews.xml",
  },
  {
    code: "US-USAT",
    name: "USA Today",
    url: "https://www.usatoday.com/rss/",
  },
  {
    code: "US-AP",
    name: "AP News",
    url: "https://feeds.apnews.com/rss/apf-topnews",
  },
  {
    code: "US-PBS",
    name: "PBS News",
    url: "https://www.pbs.org/newshour/feeds/rss/headlines",
  },
  {
    code: "US-POL",
    name: "Politico",
    url: "http://www.politico.com/rss/politicopicks.xml",
  },
  {
    code: "US-REUTUS",
    name: "Reuters US",
    url: "https://feeds.reuters.com/Reuters/domesticNews",
  },

  // IN (India) - Additional
  {
    code: "IN-TH",
    name: "The Hindu",
    url: "https://www.thehindu.com/news/national/feeder/default.rss",
  },
  {
    code: "IN-DH",
    name: "Deccan Herald",
    url: "https://www.deccanherald.com/rss",
  },
  {
    code: "IN-ZN",
    name: "Zee News",
    url: "https://zeenews.india.com/rss/india.xml",
  },
  {
    code: "IN-AJT",
    name: "Aaj Tak",
    url: "https://aajtak.intoday.in/rss/india.xml",
  },
  {
    code: "IN-IT",
    name: "India Today",
    url: "https://www.indiatoday.in/rss/1206505291.cms",
  },
  {
    code: "IN-OI",
    name: "OneIndia",
    url: "https://www.oneindia.com/rss/news-india-fb.xml",
  },
  {
    code: "IN-N18",
    name: "News18 India",
    url: "https://www.news18.com/rss/india.xml",
  },
  {
    code: "IN-DNA",
    name: "DNA India",
    url: "https://www.dnaindia.com/rss",
  },
  {
    code: "IN-ET",
    name: "The Economic Times",
    url: "https://economictimes.indiatimes.com/rssfeedsdefault.cms",
  },
  {
    code: "IN-FP",
    name: "Firstpost",
    url: "https://www.firstpost.com/rss",
  },

  // AU (Australia) - Additional
  {
    code: "AU-SMH",
    name: "The Sydney Morning Herald",
    url: "https://www.smh.com.au/rss/feed.xml",
  },
  {
    code: "AU-ABC",
    name: "ABC News",
    url: "https://www.abc.net.au/news/feed/1948/rss.xml",
  },
  {
    code: "AU-TA",
    name: "The Age",
    url: "https://www.theage.com.au/rss/feed.xml",
  },
  {
    code: "AU-PN",
    name: "PerthNow",
    url: "https://www.perthnow.com.au/news/feed",
  },
  {
    code: "AU-CT",
    name: "The Canberra Times",
    url: "https://www.canberratimes.com.au/rss.xml",
  },
  {
    code: "AU-BT",
    name: "Brisbane Times",
    url: "https://www.brisbanetimes.com.au/rss/feed.xml",
  },
  {
    code: "AU-IA",
    name: "Independent Australia",
    url: "http://feeds.feedburner.com/IndependentAustralia",
  },
  {
    code: "AU-BN",
    name: "Business News",
    url: "https://www.businessnews.com.au/rssfeed/latest.rss",
  },
  {
    code: "AU-ID",
    name: "InDaily",
    url: "https://indaily.com.au/feed/",
  },
  {
    code: "AU-TAUS",
    name: "The Australian",
    url: "https://www.theaustralian.com.au/rss",
  },

  // CA (Canada) - Additional
  {
    code: "CA-CBCN",
    name: "CBC News",
    url: "https://www.cbc.ca/cmlink/rss-topstories",
  },
  {
    code: "CA-CTVN",
    name: "CTV News",
    url: "https://www.ctvnews.ca/rss/ctvnews-ca-top-stories-public-rss-1.822009",
  },
  {
    code: "CA-FP",
    name: "Financial Post",
    url: "https://business.financialpost.com/feed/",
  },
  {
    code: "CA-NP",
    name: "National Post",
    url: "https://nationalpost.com/feed/",
  },
  {
    code: "CA-OC",
    name: "Ottawa Citizen",
    url: "https://ottawacitizen.com/feed/",
  },
  {
    code: "CA-TP",
    name: "The Province",
    url: "https://theprovince.com/feed/",
  },
  {
    code: "CA-TST",
    name: "Toronto Star",
    url: "https://www.thestar.com/content/thestar/feed.RSSManagerServlet.articles.topstories.rss",
  },
  {
    code: "CA-TSU",
    name: "Toronto Sun",
    url: "https://torontosun.com/category/news/feed",
  },
  {
    code: "CA-GM",
    name: "Globe and Mail",
    url: "https://www.theglobeandmail.com/rss/",
  },
  {
    code: "CA-MG",
    name: "Montreal Gazette",
    url: "https://montrealgazette.com/feed/",
  },

  // DE (Germany) - Additional
  {
    code: "DE-ZO",
    name: "ZEIT ONLINE",
    url: "https://www.zeit.de/gsitemaps/index.rss",
  },
  {
    code: "DE-FO",
    name: "FOCUS Online",
    url: "https://rss.focus.de/fol/XML/rss_folnews.xml",
  },
  {
    code: "DE-DW",
    name: "Deutsche Welle",
    url: "https://rss.dw.com/rdf/rss-en-all",
  },
  {
    code: "DE-SPI",
    name: "Spiegel Online",
    url: "https://www.spiegel.de/schlagzeilen/rss.xml",
  },
  {
    code: "DE-BILD",
    name: "Bild",
    url: "https://www.bild.de/rss",
  },
  {
    code: "DE-SZ",
    name: "Süddeutsche Zeitung",
    url: "https://www.sueddeutsche.de/rss",
  },
  {
    code: "DE-FAZ",
    name: "Frankfurter Allgemeine",
    url: "https://www.faz.net/rss",
  },
  {
    code: "DE-WELT",
    name: "Die Welt",
    url: "https://www.welt.de/rss",
  },
  {
    code: "DE-TAG",
    name: "Tagesschau",
    url: "https://www.tagesschau.de/xml/rss",
  },
  {
    code: "DE-HB",
    name: "Handelsblatt",
    url: "https://www.handelsblatt.com/rss",
  },

  // UK (United Kingdom) - Additional
  {
    code: "UK-BBC",
    name: "BBC News",
    url: "http://feeds.bbci.co.uk/news/rss.xml",
  },
  {
    code: "UK-GU",
    name: "The Guardian",
    url: "https://www.theguardian.com/world/rss",
  },
  {
    code: "UK-TEL",
    name: "The Telegraph",
    url: "https://www.telegraph.co.uk/rss",
  },
  {
    code: "UK-TIMES",
    name: "The Times",
    url: "https://www.thetimes.co.uk/rss",
  },
  {
    code: "UK-SKY",
    name: "Sky News",
    url: "https://feeds.skynews.com/feeds/rss/home.xml",
  },
  {
    code: "UK-ITV",
    name: "ITV News",
    url: "https://www.itv.com/news/rss",
  },
  {
    code: "UK-DM",
    name: "Daily Mail",
    url: "https://www.dailymail.co.uk/news/rss",
  },
  {
    code: "UK-IND",
    name: "The Independent",
    url: "https://www.independent.co.uk/rss",
  },
  {
    code: "UK-FT",
    name: "Financial Times",
    url: "https://www.ft.com/rss/home/uk",
  },
  {
    code: "UK-REUTUK",
    name: "Reuters UK",
    url: "https://feeds.reuters.com/reuters/UKTopNews",
  },

  // FR (France) - Additional
  {
    code: "FR-LM",
    name: "Le Monde",
    url: "https://www.lemonde.fr/rss/",
  },
  {
    code: "FR-LF",
    name: "Le Figaro",
    url: "https://www.lefigaro.fr/rss",
  },
  {
    code: "FR-F24",
    name: "France 24",
    url: "https://www.france24.com/en/rss",
  },
  {
    code: "FR-BFMTV",
    name: "BFMTV",
    url: "https://www.bfmtv.com/rss",
  },
  {
    code: "FR-LEX",
    name: "L'Express",
    url: "https://www.lexpress.fr/rss",
  },
  {
    code: "FR-LIB",
    name: "Libération",
    url: "https://www.liberation.fr/rss",
  },
  {
    code: "FR-LECH",
    name: "Les Echos",
    url: "https://www.lesechos.fr/rss",
  },
  {
    code: "FR-LP",
    name: "Le Parisien",
    url: "https://www.leparisien.fr/rss",
  },
  {
    code: "FR-RTL",
    name: "RTL",
    url: "https://www.rtl.fr/rss",
  },
  {
    code: "FR-E1",
    name: "Europe 1",
    url: "https://www.europe1.fr/rss",
  },

  // JP (Japan) - Additional
  {
    code: "JP-NHK",
    name: "NHK News",
    url: "https://www.nhk.or.jp/rss/news/cat0.xml",
  },
  {
    code: "JP-JT",
    name: "Japan Times",
    url: "https://www.japantimes.co.jp/feed/",
  },
  {
    code: "JP-ASAHI",
    name: "Asahi Shimbun",
    url: "https://www.asahi.com/rss/",
  },
  {
    code: "JP-MAIN",
    name: "Mainichi Shimbun",
    url: "https://mainichi.jp/rss/",
  },
  {
    code: "JP-YOMI",
    name: "Yomiuri Shimbun",
    url: "https://www.yomiuri.co.jp/rss/",
  },
  {
    code: "JP-NIK",
    name: "Nikkei",
    url: "https://www.nikkei.com/rss/",
  },
  {
    code: "JP-KYD",
    name: "Kyodo News",
    url: "https://english.kyodonews.net/rss/",
  },
  {
    code: "JP-JIJI",
    name: "Jiji Press",
    url: "https://www.jiji.com/rss/",
  },
  {
    code: "JP-SAN",
    name: "Sankei Shimbun",
    url: "https://www.sankei.com/rss/",
  },
  {
    code: "JP-TOKYO",
    name: "Tokyo Shimbun",
    url: "https://www.tokyo-np.co.jp/rss/",
  },

  // CN (China) - Additional
  {
    code: "CN-XIN",
    name: "Xinhua News",
    url: "https://www.xinhuanet.com/english/rss/worldrss.xml",
  },
  {
    code: "CN-CD",
    name: "China Daily",
    url: "https://www.chinadaily.com.cn/rss/",
  },
  {
    code: "CN-PD",
    name: "People's Daily",
    url: "https://en.people.cn/rss/",
  },
  {
    code: "CN-GT",
    name: "Global Times",
    url: "https://www.globaltimes.cn/rss/",
  },
  {
    code: "CN-CGTN",
    name: "CGTN",
    url: "https://news.cgtn.com/rss/",
  },
  {
    code: "CN-CCTV",
    name: "CCTV News",
    url: "https://news.cctv.com/rss/",
  },
  {
    code: "CN-SD",
    name: "Shanghai Daily",
    url: "https://www.shanghaidaily.com/rss/",
  },
  {
    code: "CN-BR",
    name: "Beijing Review",
    url: "https://www.bjreview.com/rss/",
  },
  {
    code: "CN-CO",
    name: "China.org.cn",
    url: "https://www.china.org.cn/rss/",
  },
  {
    code: "CN-SINA",
    name: "Sina News",
    url: "https://rss.sina.com.cn/news/world/focus15.xml",
  },

  // RU (Russia) - Additional
  {
    code: "RU-TASS",
    name: "TASS",
    url: "https://tass.com/rss",
  },
  {
    code: "RU-RIA",
    name: "RIA Novosti",
    url: "https://ria.ru/export/rss2/index.xml",
  },
  {
    code: "RU-INT",
    name: "Interfax",
    url: "https://www.interfax.ru/rss",
  },
  {
    code: "RU-KOM",
    name: "Kommersant",
    url: "https://www.kommersant.ru/rss",
  },
  {
    code: "RU-IZV",
    name: "Izvestia",
    url: "https://iz.ru/xml/rss",
  },
  {
    code: "RU-RG",
    name: "Rossiyskaya Gazeta",
    url: "https://rg.ru/rss",
  },
  {
    code: "RU-LENTA",
    name: "Lenta.ru",
    url: "https://lenta.ru/rss",
  },
  {
    code: "RU-RBC",
    name: "RBC",
    url: "https://www.rbc.ru/rss",
  },
  {
    code: "RU-VED",
    name: "Vedomosti",
    url: "https://www.vedomosti.ru/rss",
  },
  {
    code: "RU-NG",
    name: "Nezavisimaya Gazeta",
    url: "https://www.ng.ru/rss",
  },

  // BR (Brazil)
  {
    code: "BR-G1",
    name: "G1",
    url: "https://g1.globo.com/rss/g1/",
  },
  {
    code: "BR-FOLHA",
    name: "Folha de S.Paulo",
    url: "https://feeds.folha.uol.com.br/emcimadahora/rss091.xml",
  },
  {
    code: "BR-ESTADAO",
    name: "Estadão",
    url: "https://www.estadao.com.br/rss",
  },
  {
    code: "BR-VALOR",
    name: "Valor Econômico",
    url: "https://www.valor.com.br/rss",
  },
  {
    code: "BR-OGLOBO",
    name: "O Globo",
    url: "https://oglobo.globo.com/rss.xml",
  },
  {
    code: "BR-CORREIO",
    name: "Correio Braziliense",
    url: "https://www.correiobraziliense.com.br/rss/",
  },
  {
    code: "BR-JORNAL",
    name: "Jornal do Brasil",
    url: "https://www.jb.com.br/rss",
  },
  {
    code: "BR-BBCBR",
    name: "BBC Brasil",
    url: "https://www.bbc.com/portuguese/rss.xml",
  },
  {
    code: "BR-UOL",
    name: "UOL Notícias",
    url: "https://noticias.uol.com.br/rss/",
  },
  {
    code: "BR-R7",
    name: "R7",
    url: "https://noticias.r7.com/rss",
  },

  // MX (Mexico)
  {
    code: "MX-ELU",
    name: "El Universal",
    url: "https://www.eluniversal.com.mx/rss/",
  },
  {
    code: "MX-REFORMA",
    name: "Reforma",
    url: "https://www.reforma.com/rss",
  },
  {
    code: "MX-JORNAL",
    name: "Jornada",
    url: "https://www.jornada.com.mx/rss",
  },
  {
    code: "MX-MILENIO",
    name: "Milenio",
    url: "https://www.milenio.com/rss",
  },
  {
    code: "MX-EXCELSIOR",
    name: "Excélsior",
    url: "https://www.excelsior.com.mx/rss",
  },
  {
    code: "MX-ELFIN",
    name: "El Financiero",
    url: "https://www.elfinanciero.com.mx/rss",
  },
  {
    code: "MX-PROCESO",
    name: "Proceso",
    url: "https://www.proceso.com.mx/rss",
  },
  {
    code: "MX-ANIMAL",
    name: "Animal Político",
    url: "https://www.animalpolitico.com/rss",
  },
  {
    code: "MX-SINEMBARGO",
    name: "Sin Embargo",
    url: "https://www.sinembargo.mx/rss",
  },
  {
    code: "MX-QUAD",
    name: "Quadratín",
    url: "https://www.quadlatin.com/rss",
  },

  // AR (Argentina)
  {
    code: "AR-CLARIN",
    name: "Clarín",
    url: "https://www.clarin.com/rss/",
  },
  {
    code: "AR-LANACION",
    name: "La Nación",
    url: "https://www.lanacion.com.ar/rss",
  },
  {
    code: "AR-INF",
    name: "Infobae",
    url: "https://www.infobae.com/rss",
  },
  {
    code: "AR-PAGINA",
    name: "Página/12",
    url: "https://www.pagina12.com.ar/rss",
  },
  {
    code: "AR-CRONISTA",
    name: "El Cronista",
    url: "https://www.cronista.com/rss",
  },
  {
    code: "AR-AMBITO",
    name: "Ámbito Financiero",
    url: "https://www.ambito.com/rss",
  },
  {
    code: "AR-PERFIL",
    name: "Perfil",
    url: "https://www.perfil.com/rss",
  },
  {
    code: "AR-TN",
    name: "TN",
    url: "https://tn.com.ar/rss",
  },
  {
    code: "AR-ELDEST",
    name: "El Destape",
    url: "https://www.eldestapeweb.com/rss",
  },
  {
    code: "AR-BAE",
    name: "Buenos Aires Económico",
    url: "https://www.buenosaires.economico.com.ar/rss",
  },

  // ZA (South Africa)
  {
    code: "ZA-IOL",
    name: "IOL",
    url: "https://www.iol.co.za/rss",
  },
  {
    code: "ZA-NEWS24",
    name: "News24",
    url: "https://www.news24.com/rss",
  },
  {
    code: "ZA-CITYPRESS",
    name: "City Press",
    url: "https://www.citypress.co.za/rss",
  },
  {
    code: "ZA-THESTAR",
    name: "The Star",
    url: "https://www.iol.co.za/the-star/rss",
  },
  {
    code: "ZA-BUSINESS",
    name: "Business Day",
    url: "https://www.businesslive.co.za/rss",
  },
  {
    code: "ZA-SUNDAYTIMES",
    name: "Sunday Times",
    url: "https://www.timeslive.co.za/rss",
  },
  {
    code: "ZA-MAILGUARDIAN",
    name: "Mail & Guardian",
    url: "https://mg.co.za/rss",
  },
  {
    code: "ZA-EWN",
    name: "EWN",
    url: "https://ewn.co.za/rss",
  },
  {
    code: "ZA-SABC",
    name: "SABC News",
    url: "https://www.sabcnews.com/rss",
  },
  {
    code: "ZA-PRE",
    name: "Pretoria News",
    url: "https://www.iol.co.za/pretoria-news/rss",
  },

  // EG (Egypt)
  {
    code: "EG-AHRAM",
    name: "Al-Ahram",
    url: "https://english.ahram.org.eg/rss",
  },
  {
    code: "EG-DAILY",
    name: "Daily News Egypt",
    url: "https://dailynewsegypt.com/rss",
  },
  {
    code: "EG-MADA",
    name: "Mada Masr",
    url: "https://www.madamasr.com/rss",
  },
  {
    code: "EG-EGYPTINDEP",
    name: "Egypt Independent",
    url: "https://www.egyptindependent.com/rss",
  },
  {
    code: "EG-ALMASRY",
    name: "Al-Masry Al-Youm",
    url: "https://www.almasryalyoum.com/rss",
  },
  {
    code: "EG-SIS",
    name: "State Information Service",
    url: "https://www.sis.gov.eg/rss",
  },
  {
    code: "EG-YOUM7",
    name: "Youm7",
    url: "https://www.youm7.com/rss",
  },
  {
    code: "EG-ELWATAN",
    name: "El Watan News",
    url: "https://www.elwatannews.com/rss",
  },
  {
    code: "EG-EGYPTTODAY",
    name: "Egypt Today",
    url: "https://www.egypttoday.com/rss",
  },
  {
    code: "EG-AKH",
    name: "Akhbar El Yom",
    url: "https://www.akhbarelyom.com/rss",
  },

  // TR (Turkey)
  {
    code: "TR-HURRIYET",
    name: "Hürriyet",
    url: "https://www.hurriyet.com.tr/rss",
  },
  {
    code: "TR-SABAH",
    name: "Sabah",
    url: "https://www.sabah.com.tr/rss",
  },
  {
    code: "TR-MILLIYET",
    name: "Milliyet",
    url: "https://www.milliyet.com.tr/rss",
  },
  {
    code: "TR-ZAMAN",
    name: "Zaman",
    url: "https://www.zaman.com.tr/rss",
  },
  {
    code: "TR-CUMHURIYET",
    name: "Cumhuriyet",
    url: "https://www.cumhuriyet.com.tr/rss",
  },
  {
    code: "TR-SOZCU",
    name: "Sözcü",
    url: "https://www.sozcu.com.tr/rss",
  },
  {
    code: "TR-HABERTURK",
    name: "Habertürk",
    url: "https://www.haberturk.com/rss",
  },
  {
    code: "TR-NTV",
    name: "NTV",
    url: "https://www.ntv.com.tr/rss",
  },
  {
    code: "TR-BIANET",
    name: "Bianet",
    url: "https://www.bianet.org/rss",
  },
  {
    code: "TR-T24",
    name: "T24",
    url: "https://t24.com.tr/rss",
  },

  // SA (Saudi Arabia)
  {
    code: "SA-ARABNEWS",
    name: "Arab News",
    url: "https://www.arabnews.com/rss",
  },
  {
    code: "SA-SPA",
    name: "Saudi Press Agency",
    url: "https://www.spa.gov.sa/rss",
  },
  {
    code: "SA-OKAZ",
    name: "Okaz",
    url: "https://www.okaz.com.sa/rss",
  },
  {
    code: "SA-ALRIYADH",
    name: "Al Riyadh",
    url: "https://www.alriyadh.com/rss",
  },
  {
    code: "SA-ALJAZEERA",
    name: "Al Jazirah",
    url: "https://www.al-jazirah.com/rss",
  },
  {
    code: "SA-ALWATAN",
    name: "Al Watan",
    url: "https://www.alwatan.com.sa/rss",
  },
  {
    code: "SA-ALSHARQ",
    name: "Al Sharq",
    url: "https://www.alsharq.net.sa/rss",
  },
  {
    code: "SA-ALNAHAR",
    name: "Al Nahar",
    url: "https://www.alnahar.com.sa/rss",
  },
  {
    code: "SA-ALMADINA",
    name: "Al Madina",
    url: "https://www.al-madina.com/rss",
  },
  {
    code: "SA-ALYAUM",
    name: "Al Yaum",
    url: "https://www.alyaum.com/rss",
  },

  // IR (Iran)
  {
    code: "IR-IRNA",
    name: "IRNA",
    url: "https://www.irna.ir/rss",
  },
  {
    code: "IR-ISNA",
    name: "ISNA",
    url: "https://www.isna.ir/rss",
  },
  {
    code: "IR-TASNIM",
    name: "Tasnim News",
    url: "https://www.tasnimnews.com/rss",
  },
  {
    code: "IR-FARS",
    name: "Fars News",
    url: "https://www.farsnews.ir/rss",
  },
  {
    code: "IR-MEHR",
    name: "Mehr News",
    url: "https://www.mehrnews.com/rss",
  },
  {
    code: "IR-ILNA",
    name: "ILNA",
    url: "https://www.ilna.ir/rss",
  },
  {
    code: "IR-SHANA",
    name: "Shana",
    url: "https://www.shana.ir/rss",
  },
  {
    code: "IR-TEHRAN",
    name: "Tehran Times",
    url: "https://www.tehrantimes.com/rss",
  },
  {
    code: "IR-PRESSTV",
    name: "Press TV",
    url: "https://www.presstv.ir/rss",
  },
  {
    code: "IR-KHABAR",
    name: "Khabar Online",
    url: "https://www.khabaronline.ir/rss",
  },

  // PK (Pakistan)
  {
    code: "PK-DAWN",
    name: "Dawn",
    url: "https://www.dawn.com/rss",
  },
  {
    code: "PK-EXPRESS",
    name: "The Express Tribune",
    url: "https://tribune.com.pk/rss",
  },
  {
    code: "PK-NATION",
    name: "The Nation",
    url: "https://www.nation.com.pk/rss",
  },
  {
    code: "PK-ARY",
    name: "ARY News",
    url: "https://arynews.tv/rss",
  },
  {
    code: "PK-GEO",
    name: "Geo News",
    url: "https://www.geo.tv/rss",
  },
  {
    code: "PK-SAMA",
    name: "Samaa TV",
    url: "https://www.samaa.tv/rss",
  },
  {
    code: "PK-BOL",
    name: "Bol News",
    url: "https://www.bolnews.com/rss",
  },
  {
    code: "PK-NAWA",
    name: "Nawa-i-Waqt",
    url: "https://www.nawaiwaqt.com.pk/rss",
  },
  {
    code: "PK-JANG",
    name: "Jang",
    url: "https://jang.com.pk/rss",
  },
  {
    code: "PK-PAKISTAN",
    name: "Pakistan Today",
    url: "https://www.pakistantoday.com.pk/rss",
  },

  // BD (Bangladesh)
  {
    code: "BD-PROTHOM",
    name: "Prothom Alo",
    url: "https://www.prothomalo.com/rss",
  },
  {
    code: "BD-DHAKA",
    name: "Dhaka Tribune",
    url: "https://www.dhakatribune.com/rss",
  },
  {
    code: "BD-BDNEWS",
    name: "BDNews24",
    url: "https://bdnews24.com/rss",
  },
  {
    code: "BD-KALER",
    name: "Kaler Kantho",
    url: "https://www.kalerkantho.com/rss",
  },
  {
    code: "BD-SOMOY",
    name: "The Daily Somoy",
    url: "https://www.dailysomoy.com/rss",
  },
  {
    code: "BD-JUGANTOR",
    name: "Jugantor",
    url: "https://www.jugantor.com/rss",
  },
  {
    code: "BD-INQILAB",
    name: "Inqilab",
    url: "https://www.dailyinqilab.com/rss",
  },
  {
    code: "BD-SANG",
    name: "Sangbad",
    url: "https://www.sangbad.net/rss",
  },
  {
    code: "BD-NOYADIGANTA",
    name: "Naya Diganta",
    url: "https://www.dailynayadiganta.com/rss",
  },
  {
    code: "BD-AMAR",
    name: "Amar Desh",
    url: "https://www.amardesh.com/rss",
  },

  // MY (Malaysia)
  {
    code: "MY-STAR",
    name: "The Star",
    url: "https://www.thestar.com.my/rss",
  },
  {
    code: "MY-NST",
    name: "New Straits Times",
    url: "https://www.nst.com.my/rss",
  },
  {
    code: "MY-BERNAMA",
    name: "Bernama",
    url: "https://www.bernama.com/rss",
  },
  {
    code: "MY-MAIL",
    name: "Malay Mail",
    url: "https://www.malaymail.com/rss",
  },
  {
    code: "MY-FREEMALAYSIA",
    name: "Free Malaysia Today",
    url: "https://www.freemalaysiatoday.com/rss",
  },
  {
    code: "MY-MKINI",
    name: "Malaysiakini",
    url: "https://www.malaysiakini.com/rss",
  },
  {
    code: "MY-SINAR",
    name: "Sinar Harian",
    url: "https://www.sinarharian.com.my/rss",
  },
  {
    code: "MY-HARIAN",
    name: "Harian Metro",
    url: "https://www.hmetro.com.my/rss",
  },
  {
    code: "MY-UTUSAN",
    name: "Utusan Malaysia",
    url: "https://www.utusan.com.my/rss",
  },
  {
    code: "MY-KOSMO",
    name: "Kosmo!",
    url: "https://www.kosmo.com.my/rss",
  },

  // SG (Singapore)
  {
    code: "SG-STRAITS",
    name: "The Straits Times",
    url: "https://www.straitstimes.com/rss",
  },
  {
    code: "SG-CNA",
    name: "Channel NewsAsia",
    url: "https://www.channelnewsasia.com/rss",
  },
  {
    code: "SG-TODAY",
    name: "Today",
    url: "https://www.todayonline.com/rss",
  },
  {
    code: "SG-BUSINESS",
    name: "Business Times",
    url: "https://www.businesstimes.com.sg/rss",
  },
  {
    code: "SG-ZB",
    name: "Zaobao",
    url: "https://www.zaobao.com/rss",
  },
  {
    code: "SG-AsiaOne",
    name: "AsiaOne",
    url: "https://www.asiaone.com/rss",
  },
  {
    code: "SG-Mothership",
    name: "Mothership",
    url: "https://mothership.sg/rss",
  },
  {
    code: "SG-TNP",
    name: "The New Paper",
    url: "https://tnp.sg/rss",
  },
  {
    code: "SG-SG",
    name: "Singapore News",
    url: "https://www.gov.sg/rss",
  },
  {
    code: "SG-8Days",
    name: "8 Days",
    url: "https://www.8days.sg/rss",
  },

  // TH (Thailand)
  {
    code: "TH-BANGKOK",
    name: "Bangkok Post",
    url: "https://www.bangkokpost.com/rss",
  },
  {
    code: "TH-NATION",
    name: "The Nation",
    url: "https://www.nationthailand.com/rss",
  },
  {
    code: "TH-KHAOSOD",
    name: "Khaosod English",
    url: "https://www.khaosodenglish.com/rss",
  },
  {
    code: "TH-MATICHON",
    name: "Matichon",
    url: "https://www.matichon.co.th/rss",
  },
  {
    code: "TH-THAIRATH",
    name: "Thairath",
    url: "https://www.thairath.co.th/rss",
  },
  {
    code: "TH-BANMUANG",
    name: "Banmuang",
    url: "https://www.banmuang.co.th/rss",
  },
  {
    code: "TH-PRAJ",
    name: "Prachachat",
    url: "https://www.prachachat.net/rss",
  },
  {
    code: "TH-INTERN",
    name: "The International",
    url: "https://www.theinternational.co.th/rss",
  },
  {
    code: "TH-NAEWNA",
    name: "Naew Na",
    url: "https://www.naewna.com/rss",
  },

];

